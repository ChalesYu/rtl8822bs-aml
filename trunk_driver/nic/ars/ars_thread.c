...
wf_s32 phydm_BasicDbgMessage(ars_st *pars)
{
    ...
    ARS_THD_INFO("[RSSI]  { %d },    [RxRate] { %s%s%s%s%d%s }",
    ...
    ARS_THD_INFO("bLinked = %d, RSSI_Min = %d, CurrentIGI = 0x%x, bNoisy=%d",
...
static wf_s32 ars_thread_core(nic_info_st *nic_info)
{
    ...
    pars->SupportAbility =  ODM_RF_CALIBRATION|ODM_BB_FA_CNT|ODM_BB_RSSI_MONITOR
                            |ODM_BB_DIG
                            |ODM_BB_RA_MASK
...
static void thread_timer_handler(wf_os_api_timer_t * timer)
{
    ...
    wf_os_api_timer_set(&thread->thread_timer, 1000);
}

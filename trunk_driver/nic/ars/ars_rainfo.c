...
static wf_s32 ars_fill_mbox1_fw(nic_info_st *nic_info, wf_u8 element_id, wf_u8 *cmd, wf_u32 cmd_len)
{
    wf_s32  ret = 0;
    wf_u32 *buf = NULL;
    wf_s32 i    = 0;
...
wf_s8 phydm_rssi_report(void *ars, wdn_net_info_st *pwdn)
{
    ...
    STBC_TX = TEST_FLAG(pwdn->htpriv.tx_stbc, STBC_HT_ENABLE_TX);
    ...
    {
        //FillH2CCmd(pars->nic_info, wMBOX1_9086X_RSSI_SETTING, cmdlen, H2C_Parameter);

        ars_fill_mbox1_fw(pars->nic_info, wMBOX1_9086X_RSSI_SETTING, H2C_Parameter, cmdlen);
    }
...
wf_s32 odm_RSSIMonitorCheckCE(void *ars)
{
    ...
    for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++)
    {
        ...
        if(pwdn != NULL)
        {
            ...
            if (phydm_rssi_report(pars, pwdn) == WF_RETURN_OK)
            {
                sta_cnt++;
            }
        }
    ...
    //LOG_D("[%s] UndecoratedSmoothedPWDB:[%d ~ %d]",__func__,pars->com_save.EntryMinUndecoratedSmoothedPWDB, pars->com_save.EntryMaxUndecoratedSmoothedPWDB);
...
wf_s32 MacIdConfig(nic_info_st *nic_info, wf_u8 mac_id, wf_u8 raid, wf_u8 bw, wf_u8 sgi, wf_u32 mask)
{
    ...
    ret = ars_fill_mbox1_fw(nic_info, wMBOX1_9086X_MACID_CFG, u1wMBOX1MacIdConfigParm, wMBOX1_MACID_CFG_LEN);

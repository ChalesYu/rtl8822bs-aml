...
## o CONFIG_CHIP                   = s9083,s9188,richv300
## o CONFIG_ARS_SUPPORT            = Adaptive rate system:driver,firmware,close
################################################################################
  export WDRV_DIR ?= $(shell pwd)
  SUBDIR = mak
  PLATDIR = platform
  CT                       ?= n
  CONFIG_DRIVER_VER         = 0x0DDF
  CONFIG_OS_LINUX           = y
  CONFIG_OS_ANDROID         = n
  CONFIG_OS_RTOS            = n
...
#KSRC = /usr/src/linux-headers-5.4.124-0504124-generic/

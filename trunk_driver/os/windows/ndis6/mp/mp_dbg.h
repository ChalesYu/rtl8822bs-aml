...
*/
    ...
    #define COMP_RICHD                  0x00002000
    #define COMP_RICH_INIT              0x00004000
    #define COMP_RICH_FUNC              0x00008000
...
#define MpTrace(_Comp, _Level, Fmt)                     \
        if((_Comp & GlobalDebugComponents) && (_Level <= GlobalDebugLevel))  \
            DbgPrint Fmt;
#else
//#define MpTrace(_Comp, _Level, Fmt) DbgPrint Fmt;
#endif // 1
...
#define wf_debug(fmt,...) DbgPrint("[%s,%d]"fmt"\n",__func__,__LINE__,##__VA_ARGS__);
...
typedef enum wf_dbg_dir_e{
	DBG_DIR_TX,
	DBG_DIR_RX
}wf_dbg_dir_e;

typedef struct wf_dbg_info_s{
	WDFTIMER		counter_timer;
	void			*adapter;
	wf_u64			tx_counter;
	wf_u64			rx_counter;
	wf_u8			start_falg;
	...
	//need protect by spinlock
	KSPIN_LOCK 		lock;
	wf_u64			tx_cnt;
	wf_u64			rx_cnt;
}wf_dbg_info_t;

typedef struct wf_timer_ctx_s {
    PADAPTER padapter;
}wf_timer_ctx_t;

WDF_DECLARE_CONTEXT_TYPE_WITH_NAME(wf_timer_ctx_t, wf_get_timer_ctx)

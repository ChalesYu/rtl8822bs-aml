...
wf_inline static void lock_spin_lock (wf_lock_spin *plock)
{
    spin_lock(plock);
}

wf_inline static void lock_spin_unlock (wf_lock_spin *plock)
{
    spin_unlock(plock);
}

wf_inline static void lock_spin_init (wf_lock_spin *plock)
{
    spin_lock_init(plock);
}

wf_inline static void lock_spin_free (wf_lock_spin *plock) {}

wf_inline static void lock_bh_lock (wf_lock_spin *plock)
{
    spin_lock_bh(plock);
}

wf_inline static void lock_bh_unlock (wf_lock_spin *plock)
{
    spin_unlock_bh(plock);
}

wf_inline static void lock_irq_lock (wf_lock_spin *plock, wf_irq *pirqL)
{
    spin_lock_irqsave(plock, *pirqL);
}

wf_inline static void lock_irq_unlock (wf_lock_spin *plock, wf_irq *pirqL)
{
    spin_unlock_irqrestore(plock, *pirqL);
}

wf_inline static void lock_mutex_lock (wf_lock_mutex *mtx)
{
    mutex_lock(mtx);
}

wf_inline static int lock_mutex_try_lock (wf_lock_mutex *mtx)
{
    return mutex_trylock(mtx) ? 0 : -1;
}

wf_inline static void lock_mutex_unlock (wf_lock_mutex *mtx)
{
    mutex_unlock(mtx);
}

wf_inline static void lock_mutex_init (wf_lock_mutex *mtx)
{
    mutex_init(mtx);
}

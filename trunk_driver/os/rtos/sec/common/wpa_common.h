...
struct wpa_eapol_key
{
    ...
    wf_u8 key_info[2]; /* big endian */
    wf_u8 key_length[2]; /* big endian */
    wf_u8 replay_counter[WPA_REPLAY_COUNTER_LEN];
    wf_u8 key_nonce[WPA_NONCE_LEN];
    wf_u8 key_iv[16];
    wf_u8 key_rsc[WPA_KEY_RSC_LEN];
    wf_u8 key_id[8]; /* Reserved in IEEE 802.11i/RSN */
    wf_u8 key_mic[16];
    wf_u8 key_data_length[2]; /* big endian */
...
struct wpa_ptk
{
    wf_u8 kck[WPA_KCK_MAX_LEN]; /* EAPOL-Key Key Confirmation Key (KCK) */
    wf_u8 kek[WPA_KEK_MAX_LEN]; /* EAPOL-Key Key Encryption Key (KEK) */
    wf_u8 tk[WPA_TK_MAX_LEN]; /* Temporal Key (TK) */
    size_t kck_len;
    size_t kek_len;
    size_t tk_len;
};
...
struct wpa_ie_hdr
{
    wf_u8 elem_id;
    wf_u8 len;
    wf_u8 oui[4]; /* 24-bit OUI followed by 8-bit OUI type */
    wf_u8 version[2]; /* little endian */
} STRUCT_PACKED;
...
struct ieee80211_ht_capabilities
{
    ...
    wf_u8 a_mpdu_params; /* Maximum A-MPDU Length Exponent B0..B1

...
struct wpa_ssid_value {
	wf_u8 ssid[SSID_MAX_LEN];
	size_t ssid_len;
};

int hwaddr_aton(const char *txt, wf_u8 *addr);
int hwaddr_masked_aton(const char *txt, wf_u8 *addr, wf_u8 *mask, wf_u8 maskable);
int hwaddr_compact_aton(const char *txt, wf_u8 *addr);
int hwaddr_aton2(const char *txt, wf_u8 *addr);
int hex2byte(const char *hex);
int hexstr2bin(const char *hex, wf_u8 *buf, size_t len);
void wf_os_inc_byte_array(wf_u8 *counter, size_t len);
void wpa_get_ntp_timestamp(wf_u8 *buf);
int wpa_scnprintf(char *buf, size_t size, const char *fmt, ...);
int wpa_snprintf_hex_sep(char *buf, size_t buf_size, const wf_u8 *data, size_t len,
			 char sep);
int wpa_snprintf_hex(char *buf, size_t buf_size, const wf_u8 *data, size_t len);
int wpa_snprintf_hex_uppercase(char *buf, size_t buf_size, const wf_u8 *data,
			       size_t len);

int hwaddr_mask_txt(char *buf, size_t len, const wf_u8 *addr, const wf_u8 *mask);
...
void printf_encode(char *txt, size_t maxlen, const wf_u8 *data, size_t len);
size_t printf_decode(wf_u8 *buf, size_t maxlen, const char *str);

const char * wpa_ssid_txt(const wf_u8 *ssid, size_t ssid_len);

char * wpa_config_parse_string(const char *value, size_t *len);
int is_hex(const wf_u8 *data, size_t len);
int has_ctrl_char(const wf_u8 *data, size_t len);
int has_newline(const char *str);
size_t merge_byte_arrays(wf_u8 *res, size_t res_len,
			 const wf_u8 *src1, size_t src1_len,
			 const wf_u8 *src2, size_t src2_len);
...
int random_mac_addr(wf_u8 *addr);
int random_mac_addr_keep_oui(wf_u8 *addr);

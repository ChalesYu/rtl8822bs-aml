...
//      flag++;
        NDEV_INFO("[ndev_init]:efuse_macaddr:"WF_MAC_FMT,WF_MAC_ARG(hw_info->macAddr));
...
int ndev_open(struct net_device *ndev)
{
    ...
    nic_info_st *pnic_info;
#ifdef CONFIG_CONCURRENT_MODE
	nic_info_st *pvir_nic;
#endif
    NDEV_DBG("[NDEV] ndev_open - start");
	...
	if(pvir_nic && pvir_nic->is_up == 0)
	{
		ndev_vir_open(pvir_nic->ndev);
	}
#endif
    NDEV_DBG("[NDEV] ndev_open - end");
    return  0;
}
...
static int ndev_stop(struct net_device *ndev)
{
    ...
    NDEV_DBG("[NDEV] ndev_stop - start");

    if (nic_disable(ndev_priv->nic) == WF_RETURN_FAIL)
    {
        return -1;
    }
...
int ndev_vir_open(struct net_device *ndev)
{
    ndev_priv_st *ndev_priv;
    nic_info_st *pnic_info;
	nic_info_st *preal_nic;
    NDEV_DBG("[NDEV] ndev_vir_open - start");

    ndev_priv = netdev_priv(ndev);
    pnic_info = ndev_priv->nic;
	preal_nic = pnic_info->vir_nic;
	if(preal_nic->is_up == 0)
	{
		if(ndev_open(preal_nic->ndev) == WF_RETURN_FAIL)
		{
			return -1;
		}
	}
    ...
    NDEV_DBG("[NDEV] ndev_vir_open - end");

    return  0;
}


static int ndev_vir_stop(struct net_device *ndev)
{
    ndev_priv_st *ndev_priv;
    ndev_priv = netdev_priv(ndev);

    NDEV_DBG("[NDEV] ndev_vir_stop - start");
...
#   endif
    ...
    NDEV_DBG("[NDEV] ndev_vir_stop - end");
...
static const struct net_device_ops ndev_vir_ops =
{
    .ndo_init = ndev_init,
    .ndo_uninit = ndev_uninit,
    .ndo_open = ndev_vir_open,
    .ndo_stop = ndev_vir_stop,
    .ndo_start_xmit = ndev_start_xmit,
#if (LINUX_VERSION_CODE>=KERNEL_VERSION(2,6,35))
    .ndo_select_queue = ndev_select_queue,
#endif

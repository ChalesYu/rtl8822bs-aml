    ...
    )
{
    ...
    __try
    {
        ...
        ndisStatus = HwInitializeBus(*ppNic);//create USB device
    ...
    )
{
	...
	if (pNic->ManagementInfo.KeyProcWorkItem != NULL) {
		WdfWorkItemFlush(pNic->ManagementInfo.KeyProcWorkItem);
		WdfObjectDelete(pNic->ManagementInfo.KeyProcWorkItem);
		pNic->ManagementInfo.KeyProcWorkItem = NULL;
	}
    ...
    )
{
    ...
    __try 
    {
		...
		WDF_WORKITEM_CONFIG_INIT(&workitemConfig, HwKeyProcWorkItem);
		...
		ntStatus = WdfWorkItemCreate(&workitemConfig,
			&attributes,
			&pNic->ManagementInfo.KeyProcWorkItem);
		if (!NT_SUCCESS(ntStatus))
		{
			NT_STATUS_TO_NDIS_STATUS(ntStatus, &ndisStatus);
			NdisWriteErrorLogEntry(pNic->MiniportAdapterHandle,
		...
		nicWorkItemContext = GetWorkItemContext(pNic->ManagementInfo.KeyProcWorkItem);
		nicWorkItemContext->Nic = pNic;

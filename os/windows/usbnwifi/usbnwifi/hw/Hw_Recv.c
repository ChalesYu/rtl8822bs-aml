                  ...
                  ) 
{
    ...
    //
	//MpTrace(COMP_RECV, DBG_SERIOUS,("    RxFragListLen=%d RxTotalDescNum=%d\n",
	//                                          Nic->RxInfo.RxFragListLen,Nic->RxInfo.RxTotalDescNum));
                ...
                ) 
{
	...
	//MpTrace(COMP_RICHD, DBG_NORMAL, ("MP_RECV:%s=====>\n", __func__));   
    ...
    if (bRawReceive) {
        ...
        //
        pNicFragment->DataSize = pNicFragment->RxDescRichD.Length;
    ...
    if (pFragmentHdr->FrameControl.WEP && 
        ...
        !pNicFragment->RxDescRichD.Decrypted) {
        
        MpTrace(COMP_RICHD, DBG_NORMAL, ("MP_RECV:=====>WEP|WPA2PSK\n"));
    ...
    if (pFragmentHdr->FrameControl.WEP && 
        ...
        (Nic->CardInfo.HardwareID == HW_ID_8180 || pNicFragment->RxDescRichD.Decrypted)) {
        
        MpTrace(COMP_RICHD, DBG_NORMAL, ("MP_RECV:=====>Manually remove IV and ICV/MIC \n"));
        //
        // Clear the WEP flag.
        //
        ...
        if (pNicKey->Valid) {
            ...
            //pNicFragment->DataSize = pNicFragment->RxDescRichD.Length - overhead;
            pNicFragment->Encrypted = TRUE;
            pNicFragment->NicKey = pNicKey;

			pNicFragment->DataSize = pNicFragment->RxDescRichD.Length - IVSize;
			pNicFragment->RxBuffer = (PETH_RX_BUFFER_STRUC)(Add2Ptr(pNicFragment->RxBuffer, IVSize));
            ...
            pNicFragment->Encrypted = TRUE;
            pNicFragment->NicKey = pNicKey;
            pNicFragment->RxDescRichD.ICV = TRUE;      // Mark as an ICV error
        }
    }
    else {
        //pNicFragment->DataSize = pNicFragment->RxDesc.Length - 4;   // CRC
		pNicFragment->DataSize = pNicFragment->RxDescRichD.Length;   // CRC
                  ...
                  ) 
{
    ...
    if (pNicFragment->RxDescRichD.CRCError || pNicFragment->RxDescRichD.FOVF ) {
        pPhyStats->ullFCSErrorCount++;
        return FALSE;
    }
    else if (pNicFragment->RxDescRichD.ICV) {
...
Hw11FilterFragment(
    ...
    switch (type) {
        ...
        case DOT11_FRAME_TYPE_DATA:
			if (bAddrMatch || bIsBroadcast || bIsMulticast)
			{
				/*{
					UCHAR *temp = pktbuf;
					MpTrace(COMP_RECV, DBG_TRACE, ("ghy test can indicate "));
					for (int i = 0; i < 36; i++)
					{
						MpTrace(COMP_RECV, DBG_TRACE, ("%02x ",temp[i]));

					}
					MpTrace(COMP_RECV, DBG_TRACE, ("\n"));
				}*/
				return TRUE;
			}
			else
			{
				MpTrace(COMP_RECV, DBG_TRACE, ("Dropping the packet as it has been filtered\n"));
			}
			break;
...
{   
    return (LONG)pNicFragment->attrib.phy_info.SignalStrength; //0-100;
}
...
{   
    return(LONG)((HwRSSI(Nic,pNicFragment->RxDescRichD) + 1) >> 1); //dBm
}
               ...
               ) 
{
    ...
    HwRate = (UCHAR) (pNicFragment->RxDescRichD.RxRate); 
                      ...
                      ) 
{
    ...
    Dot11RecvContext->lRSSI = -48;//Hw11GetRSSI(Nic, pNicFragment);// Temporarily set a fixed value.
                            ...
                            ) 
{
    ...
    if (Nic->ManagementInfo.bAdvertiseBSS == FALSE) {
        ...
        //
       // MpTrace(COMP_RECV, DBG_SERIOUS,("Non-AdvertiseBSS\n"));
        return;
    }
    ...
    if (pNicFragment->RxDescRichD.CRCError  ||
        pNicFragment->RxDescRichD.ICV ||
        pNicFragment->RxDescRichD.FOVF ) {
        //
        // discard CRC packet(Except sniffer mode)
        //
        MpTrace(COMP_RECV, DBG_SERIOUS,("CRC/ICV/FOVF error\n"));
        return;
    }
    MpTrace(COMP_RECV, DBG_SERIOUS,("+++\n"));	
    if (*pktbuf == 0x40 && pNicFragment->RxDescRichD.Length > sizeof(DOT11_MGMT_HEADER)) {
        ...
        ndisStatus = Dot11CopySSIDFromMemoryBlob(pktbuf + sizeof(DOT11_MGMT_HEADER),
                                                 (ULONG)(pNicFragment->RxDescRichD.Length - sizeof(DOT11_MGMT_HEADER)),
                                                 &SSID);

        if (ndisStatus == NDIS_STATUS_SUCCESS) {
			MpTrace(COMP_RECV, DBG_SERIOUS,("kkk\n"));	
			...
			MpTrace(COMP_RECV, DBG_SERIOUS,("DDD\n"));	

            HwOnProbe(Nic, 
                      pktbuf, 
                      (USHORT)pNicFragment->RxDescRichD.Length, 
                      HwHwRateToMRate((UCHAR)((pNicFragment->RxDescRichD.RxRate))));
        }
    }
}
                  ...
                  ) 
{
    ...
    pktbuf = Add2Ptr(pNicFragment->RxBuffer, pNicFragment->DataOffset);
    pktlen = pNicFragment->RxDescRichD.Length;
...
Hw11SecurityCheck(
    ...
    if (pNicKey->AlgoId == DOT11_CIPHER_ALGO_TKIP) {
        ...
        //
		NET_BUFFER_DATA_LENGTH(NetBuffer) -= (sizeof(ReceivedMIC) + 4);
	...
	else if (pNicKey->AlgoId == DOT11_CIPHER_ALGO_WEP40 || pNicKey->AlgoId == DOT11_CIPHER_ALGO_WEP104) {
		NET_BUFFER_DATA_LENGTH(NetBuffer) -= 4;
	}
...
// RichD add.
static void do_rx_work_rssi(PNIC pNic, u32 signal)
{
	u32 last_rssi, tmp_val;
#ifdef CONFIG_NEW_SIGNAL_STAT_PROCESS
	struct signal_stat *signal_stat = &pNic->recvpriv.signal_strength_data;
#endif

	{
		...
		signal_stat->total_val += signal;
		signal_stat->avg_val = signal_stat->total_val / signal_stat->total_num;
	}
}

void do_rx_work_link_qual(PNIC       pNic, u32 linkQ)
{
...
#ifdef CONFIG_NEW_SIGNAL_STAT_PROCESS
	signal_stat = &pNic->recvpriv.signal_qual_data;
#endif
	...
	signal_stat->total_num++;
	signal_stat->total_val += linkQ;
	signal_stat->avg_val = signal_stat->total_val / signal_stat->total_num;

}

void rx_process_phy_info(PNIC       pNic, u32 * rx_buf)
{
	do_rx_work_rssi(pNic, rx_buf[0]);
	do_rx_work_link_qual(pNic, rx_buf[1]);
}

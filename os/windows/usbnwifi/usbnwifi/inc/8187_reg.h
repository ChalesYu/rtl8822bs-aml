...
typedef enum _WIRELESS_MODE {
	...
	WIRELESS_MODE_N = 0x08,
	WIRELESS_MODE_AUTO = 0x0f,
} WIRELESS_MODE;
...
typedef struct _TX_DESC_8187 {   //40 bytes
    // DWORD 1
    unsigned short      TPKTSIZE:16; // Packet length minus 4(checksum).
    unsigned short      BMC:1;     // 
    unsigned short      AGGBREAK:1;
    unsigned short      SPE_RPT:1;
    unsigned short      RSVD0:6;
    unsigned short      DATARATE_FB_CFG:1;
    unsigned short      RTSRATE_FB_CFG:1;
    unsigned short      RTSRATE_CFG:1;
    unsigned short      MINISPACE_CFG:1;
    unsigned short      AGGNUM_CFG:1;
    unsigned short      RSVD1:2;

    // DWORD 2
    unsigned short      AGG_ENABLE:1;
    unsigned short      SEC_TYPE:3;
    unsigned short      RSVD2:4;
    unsigned short      QUEUE_SEL:5;
    unsigned short      RSVD3:1;
    unsigned short      RETRY_LIMIT_EN:1;
    unsigned short      USE_RATE:1;
    unsigned short      RATE_ID:4;
    unsigned short      RATE_ADP:4;
    unsigned short      MACID:7;
    unsigned short      RSVD4:1;

    // DWORD 3
    unsigned short      SW_DEFINE:12;
    unsigned short      HWSEQ_EN:1;
    unsigned short      SEQ:12;
    unsigned short      DATA_RETRY_LIMIT:2;
    unsigned short      MBSSID:1;
    unsigned short      CTS2SELF:1;
    unsigned short      RTS_ENABLE:1;
    unsigned short      HW_RTS_ENABLE:1;
    unsigned short      RSVD5:1;
    
    // DWORD 4
    unsigned short      RSVD6:8;
    unsigned short      USB_TXAGG_NUM:8;
    unsigned short      TX_RATE:7;
    unsigned short      RSVD7:1;
    unsigned short      DATA_BW:1;
    unsigned short      DATA_SC:2;
    unsigned short      DATA_SHORT:1;
    unsigned short      RTS_SC:2;
    unsigned short      RTS_SHORT:1;
    unsigned short      RSVD8:1;
    ...
    // DWORD 8
    unsigned short      TX_DESC_CHECKSUM:16;
    unsigned short      RSVD12:16;
...
typedef struct _RX_DESC_RICHD{
    USHORT  Rxdes_seq:12;         //00-11 word2
    USHORT  RxDes_frag:4;         //12-15
    
    USHORT  Reserved1;            //16-31
    
    USHORT  Reserved2;            //00-15 word3
    USHORT  Reserved3;            //16-31

    USHORT  Rx_rate;              //00-05 word4
    USHORT  Reserved4;            //06-31

    USHORT	Reserved5;			  //00-15 word5
    USHORT	Reserved6;			  //16-31

} RX_DESC_RICHD, *PRX_DESC_RICHD;

// TODO: Adjust this struct to be more logical.
typedef struct _RX_STATUS_DESC_RICHD{
    USHORT      Length:14;         //00-13 word0
    USHORT      ICV:1;             //rx_fcs_err| rx_i_cvmic_error
    USHORT      AmsduX:1;           //15
	
    USHORT      DRVinfo:3;         //16-19 
    USHORT      SecurityType:3;    //20-22
    USHORT      Qos:1;             //23
    USHORT      SW_dec:1;          //24
    USHORT      Decrypted:1;       //25
    USHORT      PhySts_valid:1;    //26
    USHORT      FOVF:1;       //27  Now I am not sure about what kind of error it is. Set it as 0  by default.
    USHORT      LastSeg:1;         //28
    USHORT      FirstSeg:1;        //29
    USHORT      EOR:1;             //30
    USHORT      Own:1;             //31  
	
    USHORT      Status_tid:4;      //0-3 word1
    USHORT      Amsdu:1;           //04
    USHORT      Rx_moreData:1;     //05
    USHORT      Rx_moreFrag:1;     //06
	USHORT      CRCError : 1;     //07
    USHORT      Reserved3:8;       //08-15
	USHORT	     RxRate : 6;         //16-21
	USHORT	     RSSI : 7;         //22-28

    USHORT		Reserved4:3;	   //29-31
    // For phy status process.
    USHORT      notice   :1;      // 1
    USHORT      rpt_sel  :1;      // 2
    
} RX_STATUS_DESC_RICHD, *PRX_STATUS_DESC_RICHD;

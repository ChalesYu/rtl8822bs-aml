    ...
    )
{
    ...
    do
    {
        ...
        if (bNewAp)
        {
            MpTrace(COMP_SCAN, DBG_NORMAL, ("AP %02X-%02X-%02X-%02X-%02X-%02X at channel: %d (%d)\n", 
                    pBSSEntry->Dot11BSSID[0], pBSSEntry->Dot11BSSID[1], pBSSEntry->Dot11BSSID[2], 
                    pBSSEntry->Dot11BSSID[3], pBSSEntry->Dot11BSSID[4], pBSSEntry->Dot11BSSID[5],
    ...
    )
{
    ...
    pMgmtPktHeader = Hw11GetFragmentDataStart(pNicFragment);
	MpTrace(COMP_RECV, DBG_TRACE, ("****----ZY test---MgmrPktHeader-SubType--:%hu--- \n",pMgmtPktHeader->FrameControl.Subtype));
    NdisGetCurrentSystemTime((PLARGE_INTEGER)&ullHostTimeStamp);
    ...
    do
    {
        ...
        pBSSEntry->RSSI = Hw11GetCalibratedRSSI(pStation->pNic, pNicFragment) - 100;//Hw11GetRSSI(pStation->pNic, pNicFragment);//Translate percentage to dbm
        ...
        PhyType = Hw11DeterminePHYType(pStation->pNic, 
                                       ...
                                       pBSSEntry->Channel);
        // To support the N mode.
		// TODO: Current wireless mode management.
        if(pStation->dot11CurrentWirelessMode == 8)
        {
            UCHAR ucLength_PhyType = 0;
            PUCHAR pucParam_PhyType = NULL;
            ndisStatus = Dot11GetInfoEle(
                pBSSEntry->pDot11InfoElemBlob,
                pBSSEntry->InfoElemBlobSize,
                DOT11_INFO_ELEMENT_ID_MCS,
                &ucLength_PhyType,
                &pucParam_PhyType
                );
            if(ndisStatus == NDIS_STATUS_SUCCESS)PhyType = dot11_phy_type_ht;
        }
    ...
    )
{
    ...
    {
		MpTrace(COMP_ASSOC, DBG_LOUD, (" - Reject (mismatched phyid)\n"));
        return FALSE;
    }
    ...
    {
		MpTrace(COMP_ASSOC, DBG_LOUD, (" - Reject (mismatched rate)\n"));
        return FALSE;
    }
    ...
    {
		MpTrace(COMP_ASSOC, DBG_LOUD, (" - Reject (mismatched sec)\n"));
        return FALSE;
    }
...
static unsigned int proc_doAction_back_func(__in  PSTATION  pStation, PUCHAR pFrame)
{
	PDOT11_MGMT_HEADER  pMgmtMacHeader = NULL;

	UCHAR category, action;
	USHORT tid, status, reason_code = 0;
	USHORT              PacketSize;
	PUCHAR              pPacketBuffer = NULL;
	NDIS_STATUS ndisStatus;
	PUCHAR			pDot11ActionFrame;
	PUCHAR pFrameBody;
	USHORT  start_seq, size,param;
	UCHAR  accept = 0;
	ADDBA_RSP rsp;

	PacketSize = sizeof(DOT11_MGMT_HEADER) + sizeof(ADDBA_RSP);
	...
	pFrameBody = pFrame + sizeof(DOT11_MGMT_HEADER);
	category = pFrameBody[0];
	action = pFrameBody[1];
	
	MpTrace(COMP_ASSOC, DBG_NORMAL, ("%s--Action,category:%x,action:%x\n",__FUNCTION__,category,action));
	
	switch (action) {
	case WL_WLAN_ACTION_ADDBA_REQ:
		NdisMoveMemory((PUCHAR)(&(pStation->ADDBA_req)), (PUCHAR)(&(pFrameBody[2])),sizeof(ADDBA_REQ));
		//work_addba_req(pStation, (UCHAR *) & (pStation->ADDBA_req), addr,1);

		start_seq = ((pStation->ADDBA_req.BA_starting_seqctrl)) >> 4;
		param = (pStation->ADDBA_req.BA_para_set);
		tid = (param >> 2) & 0x0f;

		//MpTrace(COMP_ASSOC, DBG_NORMAL, ("%s--start_seq:%x,param:%x,tid:%x\n",__FUNCTION__,start_seq,param,tid));
		//MpTrace(COMP_ASSOC, DBG_NORMAL, ("%s--param:%x,to:%x,starseq:%x\n", __FUNCTION__, pStation->ADDBA_req.BA_para_set, pStation->ADDBA_req.BA_timeout_value,
		//	pStation->ADDBA_req.BA_starting_seqctrl));
		//MpTrace(COMP_ASSOC, DBG_NORMAL, ("%s--param:%x,to:%x,starseq:%x\n", __FUNCTION__, param, pStation->ADDBA_req.BA_timeout_value,
		//	start_seq));

		accept = 1;//RX ACTION PACK IS VAILD
		size = 64;

		rsp.CategoryCode = category;
		rsp.ActionCode = 0x1;//rsp:1  req:0
		rsp.DialogToken = pStation->ADDBA_req.dialog_token;
		if(accept == 1)
		{
			rsp.status = 0x0;//SUCCESS
		}
		else
		{
			rsp.status = htons(37);
		}
		rsp.BATOset = pStation->ADDBA_req.BA_timeout_value;
		rsp.BAParaSet = (0x02 | ((tid & 0xf)<<2) | (((USHORT)size) <<6)) ;

		pDot11ActionFrame = (PUCHAR)(pPacketBuffer + sizeof(DOT11_MGMT_HEADER));
		memcpy(pDot11ActionFrame,(PUCHAR)&rsp,sizeof(ADDBA_RSP));

		break;
#if 0
		case WL_WLAN_ACTION_ADDBA_RESP:

			status = WL_GET_LE16(&frame_body[3]);
		...
		case WL_WLAN_ACTION_DELBA:
			if ((frame_body[3] & BIT(3)) == 0) {
				...
				reason_code = WL_GET_LE16(&frame_body[4]);
	...
	pMgmtMacHeader->FrameControl.Subtype = DOT11_MGMT_SUBTYPE_MANAGEMENT_ACTION;
	...
	pStation->ConnectContext.ActiveAP->MacAddress,
	DOT11_ADDRESS_SIZE
	);
	...
	pStation->ConnectContext.ActiveAP->Dot11BSSID,
	DOT11_ADDRESS_SIZE
	);
                ...
                )
{
	...
	PUCHAR			frame_body;
	 ...
	 pPacketBuffer = Hw11GetFragmentDataStart(pNicFragment);
	 frame_body = (PUCHAR)(pPacketBuffer + sizeof(DOT11_MGMT_HEADER));
	 category = frame_body[0];

	switch (category) {
	case WL_WLAN_CATEGORY_SPECTRUM_MGMT:
		//proc_on_action_spct_func(pwadptdata, precv_frame);
		break;
	case WL_WLAN_CATEGORY_QOS:
		break;
	case WL_WLAN_CATEGORY_DLS:
		break;
	case WL_WLAN_CATEGORY_BACK:
		proc_doAction_back_func(pStation, pPacketBuffer);
		break;
	case WL_WLAN_CATEGORY_PUBLIC:
		//proc_on_action_public_func(pwadptdata, precv_frame, 1);
		break;
	case WL_WLAN_CATEGORY_RADIO_MEASUREMENT:
	case WL_WLAN_CATEGORY_FT:
	case WL_WLAN_CATEGORY_UNPROTECTED_WNM:
	case WL_WLAN_CATEGORY_SELF_PROTECTED:
		break;
	case WL_WLAN_CATEGORY_HT:
		//proc_doAction_ht_func(pwadptdata, precv_frame);
		break;
	case WL_WLAN_CATEGORY_SA_QUERY:
		break;
	case WL_WLAN_CATEGORY_WMM:
		break;
	case WL_WLAN_CATEGORY_P2P:
		//proc_doAction_p2p_func(pwadptdata, precv_frame);
		break;
	default:
		break;
	}

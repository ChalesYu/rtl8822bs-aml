...
#include "little_endian.h"
	...
	extern unsigned long HW11GetMLMEextInfoIELength(PNIC pNic);
	extern unsigned char HW11GetMLMEextInfoIEAddr(PNIC pNic);
	extern int Func_Of_Proc_Pre_Memcmp(void *dst, const void *src, u32 sz);
                               ...
                               )
{
    ...
    *pAssocPacketLength = sizeof(DOT11_MGMT_HEADER) + 
	...
	PDOT11_INFO_ELEMENT pInfoEleHdr = (PDOT11_INFO_ELEMENT)(pAPEntry->pDot11InfoElemBlob);
	ULONG EleLength = 	pAPEntry->InfoElemBlobSize;

	for (i = 0;pInfoEleHdr->Length != 0 ;) {

		if(EleLength < pInfoEleHdr->Length + sizeof(PDOT11_INFO_ELEMENT))
			break;

		switch (pInfoEleHdr->ElementID) {
			case DOT11_INFO_ELEMENT_ID_VENDOR_SPECIFIC:
			if ((Func_Of_Proc_Pre_Memcmp(((PUCHAR)pInfoEleHdr)+2, WL_WPA_OUI, 4)) ||
				(Func_Of_Proc_Pre_Memcmp(((PUCHAR)pInfoEleHdr)+2, WMM_OUI, 4)) ||
				(Func_Of_Proc_Pre_Memcmp(((PUCHAR)pInfoEleHdr)+2, WPS_OUI, 4))) {
				vs_ie_length = pInfoEleHdr->Length;
				if ((Func_Of_Proc_Pre_Memcmp(((PUCHAR)pInfoEleHdr)+2, WPS_OUI, 4))) {

					vs_ie_length = 14;
				}

				MpTrace(COMP_ASSOC, DBG_SERIOUS, ("------%s, vs length:%x\n",__func__,vs_ie_length));
			            break;
			...
			case DOT11_INFO_ELEMENT_ID_MCS:
			//if (pmlmeext->htpriv.ht_option == _TRUE) {
				//if (!(chk_ap_in_tkip(pwadptdata, 1))) {
				//	Func_Of_Proc_Pre_Memcpy(&(pmlmeinfo->HT_caps), pIE->data,
		...
		case DOT11_INFO_ELEMENT_ID_EID_EXTCapability:
		//	if (pNic->mlmepriv.htpriv.ht_option == _TRUE) {
				 ndisStatus = Dot11AttachElement(
                                   &pCurrentIE,
                                   &IESize,
                                   DOT11_INFO_ELEMENT_ID_EID_EXTCapability,
                                   pInfoEleHdr->Length,
					((PUCHAR)pInfoEleHdr)+ 2
                                   );
			        if (ndisStatus != NDIS_STATUS_SUCCESS) {
		...
		pInfoEleHdr = ((PUCHAR)pInfoEleHdr) + 2 + pInfoEleHdr->Length;
		EleLength -= pInfoEleHdr->Length + sizeof(PDOT11_INFO_ELEMENT);
...
/*
	...
	if (!(chk_ap_in_tkip(pwadptdata, 1))) {
		...
		pframe =
			ie_to_set_func(ppAssocPacket, EID_HTCapability, pIE->Length,
					   (u8 *) (&(pmlmeinfo->HT_caps)),
					   &(pattrib->pktlen));
					   */

	#if 1
		UCHAR   wmm_info[7] = { 0x00,0x50,0xf2,0x02,0x00,0x01,0x00 };
		ndisStatus = Dot11AttachElement(
                                       &pCurrentIE,
                                       &IESize,
                                       DOT11_INFO_ELEMENT_ID_VENDOR_SPECIFIC,
		...
		UCHAR	tmp[26] = {0x6e,0x19,0x1f,0xff};

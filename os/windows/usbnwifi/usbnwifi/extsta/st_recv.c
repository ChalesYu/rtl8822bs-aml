    ...
    )
{
    ...
    switch(pMgmtPktHeader->FrameControl.Subtype)
    {
        ...
        case DOT11_MGMT_SUBTYPE_DISASSOCIATION:
            ...
            break;
	case DOT11_MGMT_SUBTYPE_MANAGEMENT_ACTION:
	case DOT11_MGMT_SUBTYPE_MANAGEMENT_ACTION_NOACK:
		StaReceiveAction(
                pStation,
                pNicFragment,
                FragmentSize
    ...
    )
{
 ...
 //   USHORT				QosInfo;
	...
	SubType = (UCHAR)(((PMP_DOT11_MGMT_DATA_MAC_HEADER)pFragmentHdr)->FrameControl.Subtype);

	DbgPrint("****************************subtype:%x,cmd:%x,sub:%x\n", ((PMP_DOT11_MGMT_DATA_MAC_HEADER)pFragmentHdr)->FrameControl.Subtype,
		((PMP_DOT11_MGMT_DATA_MAC_HEADER)pFragmentHdr)->FrameControl.Type,SubType);

	//check data include qos info    add ddliu
	
	if((SubType & 0x8) != 0)//qos cmd
	{
		pQosInfo->qos_option = 1;
		pQosInfo->qos_en = 1;
		//pbuf = (char *)pFragmentHdr;
		pQosInfo->priority = ((USHORT)(*((char *)pFragmentHdr + 24)) & 0xf);
		pQosInfo->ack_policy = ((((USHORT)(*((char *)pFragmentHdr + 24)))  >> 5) & 0x3);
		pQosInfo->amsdu = ((((USHORT)(*((char *)pFragmentHdr + 24))) >> 7) & 0x1);
		//pQosInfo->priority = ((le16_to_cpu(*(unsigned short *)(pbuf+24))) & 0xf);
		//pQosInfo->ack_policy = (((le16_to_cpu(*(unsigned short *)(pbuf + 24))) >> 5) & 0x3);
		//pQosInfo->amsdu = (((le16_to_cpu(*(unsigned short *)(pbuf+24))) >> 7) & 0x1);
			
		//QosInfo = (USHORT)(*((char *)pFragmentHdr + 24));
		DbgPrint("****************************subtype:%x, qosenable:%x,priority:%x,ack_plicy:%x,amsdu:%x\n",
			((PMP_DOT11_MGMT_DATA_MAC_HEADER)pFragmentHdr)->FrameControl.Subtype, pQosInfo->qos_option, 
			pQosInfo->priority,pQosInfo->ack_policy,pQosInfo->amsdu );
	}
	else
	{
	//	pQosInfo->qos_option = 0;
	//	pQosInfo->priority = 0;
	//	pQosInfo->ack_policy = 0;
	//	pQosInfo->amsdu = 0;
	}


	if(pQosInfo->qos_option)
	{
	//
	// Data frame must also contain 802.2 LLC and 802.2 SNAP (8 bytes total) and QOS (2 bytes total)
	//

		if (FragmentSize < sizeof(DOT11_DATA_SHORT_HEADER) + sizeof(IEEE_8022_LLC_SNAP) + 2)
		{
			return NDIS_STATUS_NOT_ACCEPTED;
		}
    ...
    __try 
    {
		...
		if(pQosInfo->qos_option)
	     	{
	     		//add QOS info len
	     		EtherType = ((PIEEE_8022_LLC_SNAP) Add2Ptr(pFragmentHdr, sizeof(DOT11_QOS_DATA_SHORT_HEADER)))->sh_etype;
	     	}
		else
		{

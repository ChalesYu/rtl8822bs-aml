...
#include "Mp_Oids.tmh"
#endif
extern NDIS_STATUS wf_set_start_assoc(PADAPTER adapter, PDOT11_SSID_LIST pDot11SSIDList);
    ...
    )
{
    ...
    *BytesWritten = sizeof(DOT11_STATUS_INDICATION);
	...
	wf_mlme_deauth (pAdapter->nic_info);
    ...
    )
{
    ...
    do
    {
        ...
        *BytesNeeded = 0;
        ...
        uValue = Hw11QueryATIMWindow(pAdapter);
        NdisMoveMemory(
            InformationBuffer,
            &uValue,
            sizeof(ULONG)
    ...
    )
{
    ...
    do
    {
        ...
        *BytesNeeded = 0;
        ...
        value = Hw11QueryNicPowerState(pAdapter, TRUE);
    ...
    )
{
    ...
    do
    {
        ...
        *((PBOOLEAN)InformationBuffer) = Hw11QueryCFPollable(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *BytesWritten = sizeof(DOT11_RATE_SET);
        ...
        Hw11QueryOperationalRateSet(pAdapter, pDot11RateSet, TRUE);
    } while(FALSE);
    ...
    )
{
    ...
    do
    {
        ...
        *puValue = Hw11QueryRTSThreshold(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *puValue = Hw11QueryShortRetryLimit(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *puValue = Hw11QueryLongRetryLimit(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *BytesNeeded = 0;
        ...
        uFragmentationThreshold = Hw11GetFragmentationThreshold(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *puValue = Hw11QueryMaxReceiveLifeTime(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *puValue = Hw11QueryMaxTXMSDULifeTime(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *BytesNeeded = 0;
        ...
        //TBD
        //*puValue = Hw11QueryCurrentRegDomain(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *pDot11DiversitySupport = Hw11QueryDiversitySupport(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *puValue = Hw11QueryCurrentTXPowerLevel(pAdapter);
    ...
    )
{
    ...
    LOG_D("=> SetPacketFilter ADAPT %p, Filter %x", pAdapter, PacketFilter);
    ...
    LOG_D("<= SetPacketFilter ADAPTER %p, ndisStatus %x", pAdapter, ndisStatus);
    return(ndisStatus);
}
    ...
    )
{
    ...
    do
    {
        ...
        *BytesRead = sizeof(DOT11_CURRENT_OPERATION_MODE);
		...
		pAdapter->MibInfo.CurrentOperationMode.uCurrentOpMode = pCurrentOperationMode->uCurrentOpMode;
    ...
    )
{
    ...
    do
    {
        ...
        {
            MP_RELEASE_SEND_LOCK(pAdapter, FALSE);
            LOG_E("Miniport is currently pausing. Cannot scan");
        ...
        {
            MP_RELEASE_SEND_LOCK(pAdapter, FALSE);
            LOG_D("Miniport is in reset. Cannot perform scan");
            ...
            (pAdapter->OperationMode == DOT11_OPERATION_MODE_NETWORK_MONITOR))
        {
            ...
            MP_RELEASE_SEND_LOCK(pAdapter, FALSE);
            LOG_E("Failure! There is a scan already in progress.");
            ndisStatus = NDIS_STATUS_DOT11_MEDIA_IN_USE;
            break;
        }
        ...
        *BytesNeeded = 0;
		...
		//TBD 
		{
			*BytesNeeded = FIELD_OFFSET(DOT11_CIPHER_ALGORITHM_LIST, AlgorithmIds) +
				pAdapter->MibInfo.UnicastCipherAlgorithmCount * sizeof(DOT11_CIPHER_ALGORITHM);
			...
			for (int cipher_num = 0; cipher_num < pAdapter->MibInfo.UnicastCipherAlgorithmCount; cipher_num++)
			{
				pAuthCipherList->AlgorithmIds[cipher_num] = pAdapter->MibInfo.UnicastCipherAlgorithmList[cipher_num];
			}
			pAuthCipherList->uNumOfEntries = pAdapter->MibInfo.UnicastCipherAlgorithmCount;
			pAuthCipherList->uTotalNumOfEntries = pAdapter->MibInfo.UnicastCipherAlgorithmCount;
		}
...
{
	return 0;//Sta11FlushBSSList(pAdapter->Station);
}
    ...
    )
{
    ...
    do
    {
        ...
        *BytesNeeded = pDot11SSIDList->uNumOfEntries * sizeof(DOT11_SSID) +
		...
		memcpy(&pAdapter->MibInfo.curr_ssid, pDot11SSIDList, sizeof(DOT11_SSID_LIST));
    ...
    do
    {
        ...
        *BytesNeeded = 0;
        ...
        if (pAdapter->Dot11RunningMode != RUNNING_MODE_STA_ADHOC) {
			...
			if (pAdapter->MibInfo.curr_ssid.SSIDs[0].uSSIDLength == 0)
			{
				DbgPrint("get ssid fail\n");
				return NDIS_STATUS_FAILURE;
			}
			ndisStatus = wf_set_start_assoc(pAdapter, &pAdapter->MibInfo.curr_ssid);
    ...
    )
{
    ...
    do
    {
        ...
        *BytesNeeded = 0;
        ...
        if (pAdapter->Dot11RunningMode == RUNNING_MODE_STA_INFRASTRUCTURE) {
            // Pass the request to the station
            ndisStatus = Sta11DisconnectInfra(pAdapter);
    ...
    )
{
    ...
    do
    {
        ...
        *BytesRead = sizeof(ULONG);
		...
		if (PhyId >= pAdapter->MibInfo.pSupportedPhyTypes->uNumOfEntries)
		{
			return NDIS_STATUS_INVALID_DATA;
		}
		else
		{
			...
			pAdapter->MibInfo.SelectedPhyId = PhyId;
			pAdapter->MibInfo.SelectedPhyMIB = & (pAdapter->MibInfo.PhyMIB[PhyId]);
			return NDIS_STATUS_SUCCESS;
		}
    } while(FALSE);
...
//    MpEntry;
    ...
    *BytesNeeded = 0;
    ...
    case OID_GEN_SUPPORTED_GUIDS:
        ...
        //
        ndisStatus = Hw11QuerySupportedGUIDS(pAdapter);
        break;

   ...
   )
{
   ...
   if (pMemoryBlockHeader->ulHeaderPattern != MP_HEADER_PATTERN)
   {
      ...
      {
         LOG_D("Possible double free of memory block at %p", pMemoryBlockHeader);
      }
      else
      {
         LOG_D("Memory corruption due to underflow detected at memory block %p", pMemoryBlockHeader);
      }
      
      LOG_D("Dumping information about memory block. This information may itself have been corrupted and could cause machine to bugcheck.");
      LOG_D("Memory was allocated from %s at line %d", pMemoryBlockHeader->pstrFile, pMemoryBlockHeader->ulLength);
      MPASSERT (FALSE);
   }
   ...
   {
      LOG_E("Memory corruption due to overflow detected at %p", pMemory);
      LOG_E("Dumping information about memory block. This information may itself have been corrupted and could cause machine to bugcheck.");
      LOG_E("Memory was allocated from %s at line %d", pMemoryBlockHeader->pstrFile, pMemoryBlockHeader->ulLength);
      MPASSERT (FALSE);
   }
      ...
      NdisAcquireSpinLock (&GlobalMemoryLock);   
      {
         if (!IsListEmpty (&GlobalMemoryList))
         {
            ...
            while (!IsListEmpty (&GlobalMemoryList))
            {
               ...
               LOG_D("LEAK in %s on line %u", pCurrentMemory->pstrFile, pCurrentMemory->ulLine);
...
ULONG       GlobalDebugLevel = DBG_TRACE;    //todo
#if 0
ULONG       GlobalDebugComponents = (COMP_RICHD|COMP_RICH_INIT|COMP_RICH_FUNC);

#else
ULONG       GlobalDebugComponents = (COMP_EVENTS | COMP_SEND | COMP_RECV  | COMP_DBG   | COMP_SCAN | COMP_INIT_PNP | 
                                     COMP_POWER | COMP_ASSOC | COMP_MISC | COMP_OID | COMP_PS_PACKETS)|(COMP_RICHD|COMP_RICH_INIT|COMP_RICH_FUNC);
#endif
    ...
    )
{
    ...
    if (ndisStatus == NDIS_STATUS_SUCCESS)
    {
        ...
        LOG_D("GlobalDebugLevel = 0x%x", GlobalDebugLevel);
    }
    ...
    if (ndisStatus == NDIS_STATUS_SUCCESS)
    {
        ...
        LOG_D("GlobalDebugComponents = 0x%x", GlobalDebugComponents);
    }

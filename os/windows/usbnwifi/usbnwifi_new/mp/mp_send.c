    ...
    )
{
    ...
    do
    {
        ...
        if (pMpTxd == NULL)
        {
            ...
            LOG_E("Failed to allocate the requested number (%d) of TX_MSDUs.", pAdapter->TotalTxMSDUs);
            break;
        }
        NdisZeroMemory(pMpTxd, (pAdapter->TotalTxMSDUs * sizeof(MP_TX_MSDU)));
        
        //
        // TODO: Let the NIC allocate the resources it needs for sending.
        // These would include shared memory etc.  2021/03/03
        //
        ndisStatus = HwBusAllocateXmitResources(pAdapter, 0); // We have to decide a suitable number in the future. The default value is 88.  
		if (ndisStatus != NDIS_STATUS_SUCCESS)
        {
            LOG_E("Allocate bus xmit resources failed!");
            break;
        }
    ...
    )
{
   ...
   // USHORT          i, SGElemCount;
    ...
    LOG_D("Send Complete Tx MSDU %p", pTxd);
    ...
    )
{
    ...
    {
        MPVERIFY(pTxd);
        LOG_D("Cancelling TX_MSDU %p associated with NBL %p", pTxd, pTxd->NetBufferList);
...
extern void wf_xmit_ndis_nbl(void *miniport_ctx, void *nbl, ULONG flag);
    ...
    )
{
	...
	wf_xmit_ndis_nbl(MiniportAdapterContext, NetBufferList, SendFlags);
	return;
    ...
    if (pendingSends > 0 /*&& Hw11TxMSDUIsComplete(pAdapter->Nic, pTxd->NicTxd)*/)
    {
        do
        {
            ...
            MpPrepareTxMSDUForCompletion(
                pAdapter,
                pTxd,
                0/*Hw11TxMSDUTransmissionStatus(pAdapter->Nic, pTxd->NicTxd)*/
                );
    ...
    )
{
         ...
         MP_ADAPTER_CAN_SEND_PACKETS(pAdapter))
    {
                ...
                NumPacketsToTransmit < MP_MAX_PENDING_TX_PACKETS )
        {
            ...
            if (1/*Hw11CanTransmitNBL(pAdapter->Nic, NetBufferList, Dot11SendContext)*/)
            {
                //
                // Lets reserve a TX_MSDU for this packet. Reservation guarantees that no
                // one else will use the reserved TX_MSDU and avoid synchronisation issues.

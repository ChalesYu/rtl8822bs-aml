...
void print_reg_val(PADAPTER pAdapter, wf_u32 addr)
{
	...
	value = wf_usb_read8(pAdapter, addr, NULL);
...
int hw_pwr_on_v200(PADAPTER pAdapter)
{
...
#if 1
	temp = wf_usb_read8(pAdapter, 0xac, NULL);
	temp &= 0xEF;
	wf_usb_write8(pAdapter, 0xac, temp);
	...
	print_reg_val(pAdapter, 0xac);
	// д0x_00F8[4]  bit 4 д1 
	temp = wf_usb_read8(pAdapter, 0xf8, NULL);
	temp |= 0x10;
	temp |= 0x40;      // 
...
{
	nic_disable(pAdapter->nic_info);
	nic_shutdown(pAdapter->nic_info);
	nic_term(pAdapter->nic_info);

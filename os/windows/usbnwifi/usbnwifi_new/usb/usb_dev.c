...
void wf_usb_xmit_complet_callback(void *pnic_info, void *frame)
{
	...
	InterlockedDecrement(&ppend->cnt);
	InterlockedIncrement(&pfree->cnt);

	ExInterlockedInsertTailList(&pfree->head, &usb_req->list, &pfree->lock);
}
...
NTSTATUS wf_usb_xmit_pkt_sync(PADAPTER         adapter, wf_usb_req_t *req)
{
	...
	if(WdfRequestSend(writeRequest, writeContext->IoTarget, &usb_info->request_option) == FALSE) {   // Failure - Return request
...
int wf_usb_xmit_insert(void *adapter,wf_u8 agg_num,char *buff, wf_u32 buff_len, wf_u32 addr, int (*tx_callback_func)(void*tx_info, void *param), void *tx_info, void *param)
{
	...
	plist = ExInterlockedRemoveHeadList(&pfree->head, &pfree->lock);
	if(plist == NULL) {
	...
	InterlockedIncrement(&ppend->cnt);
	InterlockedDecrement(&pfree->cnt);
	ExInterlockedInsertTailList(&ppend->head, &usb_req->list, &ppend->lock);
	...
	while(1) {
		...
		while(!IsListEmpty(&pmgmt_pend->head) || !IsListEmpty(&pdata_pend->head)) {
			...
			if(!IsListEmpty(&pmgmt_pend->head)) {
				plist = ExInterlockedRemoveHeadList(&pmgmt_pend->head, &pmgmt_pend->lock);
			} else if(!IsListEmpty(&pdata_pend->head)) {
				plist = ExInterlockedRemoveHeadList(&pdata_pend->head, &pdata_pend->lock);
			} else {
				break;
			}
			if(plist == NULL) {
			...
			if(usb_req->data_buf == NULL) {
				...
				if(usb_req->addr == 1)
					ExInterlockedInsertTailList(&pdata_free->head, &usb_req->list, &pdata_free->lock);
				else
					ExInterlockedInsertTailList(&pmgmt_free->head, &usb_req->list, &pmgmt_free->lock);
				continue;
			}
...
void wf_usb_bulk_req_option_init(wf_usb_info_t *usb_info)
{
	WDF_REQUEST_SEND_OPTIONS_INIT(
                              &usb_info->request_option,
                              0
                              );
...
NDIS_STATUS wf_usb_xmit_init(void *param)
{
	...
	//bulk transport init
	wf_usb_bulk_req_option_init(usb_info);
	...
	for(i=0; i<USB_DATA_REQ_DEPTH; i++) {
		...
		InterlockedIncrement(&pdata_free->cnt);
		ExInterlockedInsertTailList(&pdata_free->head, &usb_req->list, &pdata_free->lock);
	}
	...
	for(i=USB_DATA_REQ_DEPTH; i<USB_REQ_QUE_DEPTH; i++) {
		...
		InterlockedIncrement(&pmgmt_free->cnt);
		ExInterlockedInsertTailList(&pmgmt_free->head, &usb_req->list, &pmgmt_free->lock);
	}
...
void wf_usb_xmit_deinit(void *param)
{
	...
	for(i=0; i<4; i++) {
		while(!IsListEmpty(&pque[i]->head)) {
			plist = ExInterlockedRemoveHeadList(&pque[i]->head, &pque[i]->lock);
			usb_req = CONTAINING_RECORD(plist, wf_usb_req_t, list);

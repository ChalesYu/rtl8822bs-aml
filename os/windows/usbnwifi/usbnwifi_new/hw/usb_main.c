...
--*/
{
    ...
    attributes.ParentObject = Mp11GetWdfDevice( adapter );
...

{
    ...
    requestAttributes.ParentObject = Mp11GetWdfDevice(adapter);
...

{
    ...
    ReturnControlRequest(vendorRequestContext->NdisContext, Request);
...
--*/
{
    ...
    LOG_D( "--> HwUsbSendVendorControlPacketAsync");
    controlRequest = GetControlRequest(adapter);
    
    if (controlRequest == NULL) {   // No requests left
        LOG_E("HwUsbSendVendorControlPacketAsync Out of control request");
...
error:
    ReturnControlRequest(adapter, controlRequest);

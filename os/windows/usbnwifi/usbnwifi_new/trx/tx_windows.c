...
NDIS_STATUS wf_xmit_pkt_wlan2eth(PADAPTER padapter, void *ppkt)
{
	...
	wf_u8 mac_hdr[DATA_FRAME_HDR_SHORT] = {0};

	//save 802.11 mac header
	NdisMoveMemory(mac_hdr, pkt->ptempbuf, DATA_FRAME_HDR_SHORT);
...
static int wf_xmit_send_complete(nic_info_st *nic_info, wf_xmit_pkt_t *pkt)
{
...
void wf_xmit_ndis_nbl(NDIS_HANDLE mp_ctx, PNET_BUFFER_LIST nbl, NDIS_PORT_NUMBER port_number, ULONG flag)
{
    ...
    for(curr_nbl = nbl, next_nbl = curr_nbl->Next; curr_nbl != NULL; curr_nbl = next_nbl) {
		...
		ExInterlockedInsertTailList(&ppend->head, &pkt->list, &ppend->lock);
...
void wf_xmit_data_thread(PADAPTER         padapter)
{
	...
	while(1) {
		...
		while(!IsListEmpty(&ppend->head)) {
			...
			plist = ExInterlockedRemoveHeadList(&ppend->head, &ppend->lock);
			if(plist == NULL) {
			...
			if (mlme_info->link_info.num_tx_ok_in_period_with_tid[pkt->xframe.qsel] > 100 && (hw_info->ba_enable == wf_true)) {
	            ...
	            if (addbaRet == 0) {
					...
					ExInterlockedInsertHeadList(&ppend->head, &pkt->list, &ppend->lock);
					break;
	            }
	        }
...
NDIS_STATUS wf_xmit_thread_create(PADAPTER pAdapter)
{
	wf_xmit_info_t *xmit_info = pAdapter->xmit_info;

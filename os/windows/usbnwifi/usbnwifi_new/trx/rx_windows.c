...
NDIS_STATUS wf_recv_release_source(PADAPTER          padapter, wf_recv_pkt_t *pkt)
{
	...
	if(pkt->src_buffer != NULL) {
		pkt->src_buffer = NULL;
		pkt->buffer = NULL;
		ExInterlockedInsertHeadList(&pfree->head, &pkt->list, &pfree->lock);
	}
	...
	while(1){
		...
		if((pkt_type != WF_PKT_TYPE_FRAME) ||
			...
			if(pkt->nic_pkt.pkt_info.frame_type == MAC_FRAME_TYPE_DATA) {
				ExInterlockedInsertTailList(&pdata->head, &pkt->list, &pdata->lock);
			} else if(pkt->nic_pkt.pkt_info.frame_type == MAC_FRAME_TYPE_MGT) {
				ExInterlockedInsertTailList(&pmgmt->head, &pkt->list, &pmgmt->lock);
			}
	...
	while(1) {
		...
		while(!IsListEmpty(&pmgmt->head) || !IsListEmpty(&pdata->head)) {
			...
			if(!IsListEmpty(&pmgmt->head)) {
				plist = ExInterlockedRemoveHeadList(&pmgmt->head, &pmgmt->lock);
			} else if(!IsListEmpty(&pdata->head)) {
				plist = ExInterlockedRemoveHeadList(&pdata->head, &pdata->lock);
			} else {
...
NDIS_STATUS wf_recv_thread_create(PADAPTER pAdapter)
{
	wf_recv_info_t *recv_info = pAdapter->recv_info;

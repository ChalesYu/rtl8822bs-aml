...
static rt_err_t rt_wlan_scan_result_cache(struct rt_wlan_info *info, int timeout)
{
    ...
    if (scan_filter)
    {
        ...
        rt_hw_interrupt_enable(level);
        if (rt_wlan_info_isequ(&_tmp_info, info))
        {
...
struct rt_wlan_scan_result *rt_wlan_scan_sync(const char *ssid)
{
    struct rt_wlan_scan_result *result;
    struct rt_wlan_info info;

    rt_memcpy(&info.ssid.val, ssid, rt_strlen(ssid));
    info.ssid.len = rt_strlen(ssid);
    info.hidden = 1;
    /* Execute synchronous scan function */
    MGNT_LOCK();
    result = rt_wlan_scan_with_info(&info);
    MGNT_UNLOCK();
    return result;
}

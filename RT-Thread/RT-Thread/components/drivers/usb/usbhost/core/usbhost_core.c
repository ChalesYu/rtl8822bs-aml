...
//    /* reset bus */
//    ret = rt_usbh_hub_reset_port(device->parent_hub, device->port);
//    if(ret != RT_EOK) {
//      rt_kprintf("usb hub reset port failed\n");
//      return ret;
//    }
//    //rt_thread_mdelay(10);
//    ret = rt_usbh_hub_clear_port_feature(device->parent_hub, i + 1, PORT_FEAT_C_CONNECTION);
//    if(ret != RT_EOK) {
//      rt_kprintf("usb hub clear port feature failed\n");
//      return ret;
//    }
...
int rt_usb_hcd_pipe_xfer(uhcd_t hcd, upipe_t pipe, void* buffer, int nbytes, int timeout)
{
    ...
    return hcd->ops->pipe_xfer(pipe, USBH_PID_DATA, pbuffer, nbytes, timeout);

...
#define P2P_NEGO_TIME (5000) //ms

wf_s32 wf_p2p_nego_timer_set(nic_info_st *pnic_info)
{
    ...
    if(p2p_info->go_negoing && 0 == p2p_info->nego_timer_flag)
    {
        p2p_info->nego_timer_flag = 1;
        wf_os_api_timer_set(&p2p_info->nego_timer, P2P_NEGO_TIME);
    }
...
wf_s32 wf_p2p_cannel_remain_on_channel(nic_info_st *pnic_info)
{
    ...
    if( 0 == p2p_info->ro_ch_again)
    {
        ...
        if(buddy_ch)
        {
            ...
            {
                if(hw_ch != p2p_info->link_channel)
                {
                    wf_hw_info_set_channnel_bw(pnic_info,  p2p_info->link_channel, cw, offset);
                }
            }

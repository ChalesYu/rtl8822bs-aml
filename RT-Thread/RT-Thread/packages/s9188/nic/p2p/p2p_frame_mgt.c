...
static void p2p_adjust_channel(nic_info_st *pnic_info, wf_u8 * frame_body, wf_u32 len, wf_u8 flag)
{
    ...
    P2P_FRAME_INFO("buddy_ch:%d",buddy_ch);
    ...
    while (p2p_ie) 
    {
        ...
        if (pattr) 
        {
            ...
            wf_u32 num_of_ch;
            wf_u8 *pattr_temp = pattr + 3;
            P2P_FRAME_INFO("attr_contentlen:%d",attr_contentlen);
        ...
        {
            P2P_FRAME_INFO("no P2P_ATTR_CH_LIST");
        }
        ...
        {
            P2P_FRAME_INFO("no P2P_ATTR_OPERATING_CH");
        }
...
wf_s32 p2p_check_nego_req(nic_info_st *pnic_info, p2p_frame_check_param_st *check_param)
{
    ...
    P2P_FRAME_INFO("%s(%d):P2P_GO_NEGO_REQ, dialogToken=%d, intent:%u%s, listen_ch:%d, op_ch:%d, ch_list:%s, capability:0x%x, go_t:%d,gc_t:%d\n",
         (check_param->is_tx == wf_true) ? "Tx" : "Rx", p2p_info->remain_ch,check_param->dialogToken,
...
wf_s32 p2p_check_nego_confirm(nic_info_st *pnic_info, p2p_frame_check_param_st *check_param)
{
    ...
    P2P_FRAME_INFO("WF_%s(%d):P2P_GO_NEGO_CONF, dialogToken=%d, status:%d, op_ch:%d, ch_list:%s\n",
         (check_param->is_tx == wf_true) ? "Tx" : "Rx", p2p_info->remain_ch,
...
wf_s32 wf_p2p_proc_rx_action(nic_info_st *pnic_info, wf_u8 *frame, wf_u32 len)
{
    ...
    cur_ch = wf_wlan_get_cur_channel(pnic_info);
    if (p2p_info->remain_ch != cur_ch)
    {

...
rt_inline void upload_skb(nic_info_st *nic_info, void * pskbv)
{
    int ret;
    wf_u16 eth_type;
    struct rt_wlan_device *wlan = nic_info->ndev;
...
struct pbuf *alloc_msdu(const wf_u8 *da, const wf_u8 *sa, wf_u8 *msdu,wf_u16 msdu_len)
{
...
#if ETH_PAD_SIZE
    pbuf_header(sub_pbuf, -ETH_PAD_SIZE);
#endif
    ...
    else
    {
        ...
        wf_memcpy((wf_u8 *)sub_pbuf->payload + 2, sa, WF_ETH_ALEN);
...
static void rx_data_upload_process(rx_pkt_t *pkt,struct rt_wlan_device *wlan)
{
    ...
    else
    {
        ...
        if (ret == 0)
        {
            upload_skb(wlan_priv->nic, pkt->pskb);
        }
        else
        {
            if (pkt->pskb)

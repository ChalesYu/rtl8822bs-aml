...
#ifndef WIFI_FW_DIR
#define WIFI_FW_DIR   "/lib/firmware/fw_s9088.bin"
#endif
...
int hif_dev_insert(hif_node_st *hif_info)
{
  ...
  for(i=0;i<nic_num;i++)
  {
    ...
    if(ret != 0)
    {
      LOG_E("[%s] wlan_register nic[%d] failed",__func__, i);
      return -1;
    }
...
int hif_write_cmd(void *node, wf_u32 cmd, wf_u32 *send_buf, wf_u32 send_len, wf_u32 *recv_buf, wf_u32 recv_len)
{
  ...
  ret = wf_tx_queue_insert(hif_node, 1, (char *)ptx_desc, snd_pktLen, CMD_QUEUE_INX, NULL, NULL, NULL);
  if(ret != 0)
  {

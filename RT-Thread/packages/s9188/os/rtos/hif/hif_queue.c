...
static void wf_rx_workqueue(wf_work_struct *work);
...
data_queue_node_t *wf_data_queue_remove(wf_que_t *queue)
{
    wf_list_t *head = NULL;
    ...
    if (NULL != head)
    {
        return wf_list_entry(head, data_queue_node_t, node);
    }

    return NULL;
}

data_queue_node_t *wf_queue_node_malloc(int cnt)
{
    data_queue_node_t *node = NULL;

    node = wf_kzalloc(cnt * sizeof(data_queue_node_t));
    if (NULL == node)
    {
...
static hif_netbuf_t *wf_netbuf_node_malloc(int cnt )
{
    hif_netbuf_t *node = NULL;

    node = wf_kzalloc(cnt * sizeof(hif_netbuf_t));
    if (NULL == node)
    {
        LOG_E("[%s] wf_kzalloc failed , check!!!!", __func__);
        return NULL;
...
hif_netbuf_t *wf_netbuf_queue_remove(wf_que_t *queue)
{
    ...
    if (NULL != head)
    {
        return wf_list_entry(head, hif_netbuf_t, node);
    }
...
int wf_data_queue_mngt_init(void *hif_node)
{
    ...
    data_queue_node_t *recv_node            = NULL;
    hif_node_t *hif_info                    = (hif_node_t *)hif_node;
    data_queue_mngt_t *data_queue_mngt      = &hif_info->trx_pipe;
    wf_workqueue_func_param_st wq_rx_param  = {"rxwque", wf_rx_workqueue};
...
int wf_data_queue_mngt_term(void *hif_node)
{
    ...
    wf_list_t *next = NULL;
    hif_netbuf_t *netbuf_node = NULL;
    data_queue_node_t *data_node = NULL;
    hif_node_t  *hif_info = (hif_node_t  *)hif_node;
    data_queue_mngt_t *trxq = &hif_info->trx_pipe;
    ...
    {
        data_node = wf_list_entry(pos, data_queue_node_t, node);
        if (data_node)
        {
            hif_info  = (hif_node_t  *)data_node->hif_node;
//            if (HIF_USB == hif_info->hif_type)
//            {
...
//            }
    ...
    {
        netbuf_node = wf_list_entry(pos, hif_netbuf_t, node);
        if (netbuf_node)
        {
...
{
    wf_s32 ret = 0;
    data_queue_node_t qnode_tx = {0};
    hif_node_t *hif_node       = hif_info;
    data_queue_node_t *qnode = &qnode_tx;
...
int hif_frame_dispath(hif_node_t *hif_info, hif_netbuf_t *netbuf_node)
{
    int num = 0;
    hw_info_st *hw_info;
    wf_u8 *pdata = NULL;
...
static void wf_rx_workqueue(wf_work_struct *work)
{
    data_queue_mngt_t *dqm          = NULL;
    hif_netbuf_t *netbuf_node       = NULL;
    data_queue_mngt_t *trxq         = NULL;
    wf_que_t *data_queue            = NULL;
    data_queue_node_t *qnode        = NULL;
    hif_node_t *hif_info            = NULL;
    wf_workqueue_mgnt_st *wq_mgt    = NULL;

    wq_mgt = OS_CONTAINER_OF(work, wf_workqueue_mgnt_st, work);
    dqm = OS_CONTAINER_OF(wq_mgt, data_queue_mngt_t, rx_wq);
    if (NULL == dqm)
    {
        return;
    }

    hif_info = OS_CONTAINER_OF(dqm, hif_node_t, trx_pipe);
    if (NULL == hif_info)
    {
        return;
    }
...
wf_s32 wf_hif_queue_disable(hif_node_t *hif_node)
{
    return WF_RETURN_OK;
}

...
static void rx_data_reorder_process(rx_pkt_t *pkt, wlan_dev_t *wdev);
...
OS_STATIC_INLINE int create_pbuf(prx_pkt_t prx_pkt, wf_u8 *pbuf, wf_u32 pkt_len)
{
...
OS_STATIC_INLINE wf_u8 *pkt_pull(prx_pkt_t prx_pkt, int size)
{
...
OS_STATIC_INLINE wf_u8 *pkt_trim(prx_pkt_t prx_pkt, int size)
{
    wf_u8 *ptmp;
    struct pbuf *p = (struct pbuf *)prx_pkt->pskb;
...
OS_STATIC_INLINE void free_pbuf_func(void **p)
{
//    pbuf_free(*p);
    *p = NULL;
}

OS_STATIC_INLINE void wf_rx_eapol_handle(wlan_dev_t *wdev, void *pskbv)
{
    wf_u8 *frame;
    struct pbuf *p = pskbv;
...
//        pbuf_free(p);
    ...
    if (p->len == p->tot_len)
    {
        frame = (wf_u8 *)p->payload;
//        wf_wlan_dev_report_eapol(wdev, frame, p->tot_len);
...
OS_STATIC_INLINE void upload_skb(void *pndev, void *pskbv)
{
    int ret;
    wf_u16 eth_type;
    nic_info_st *nic_info;
    wlan_dev_t *wdev = pndev;
...
//        pbuf_free(p);
    ...
    if (wf_local_cfg_get_work_mode(nic_info) == WF_MONITOR_MODE)
    {
        ...
        wlan_dev_promisc_handler(wdev, p, p->tot_len);
//        pbuf_free(p);
        return;
    }
#endif
    ...
    {
        if ((wf_memcmp(hw_info->macAddr, pethhdr->dest, WF_ETH_ALEN) == 0) ||
            (WF_IS_MCAST(pethhdr->dest)))
        {
            if (0x888e == eth_type)
            {
                wf_rx_eapol_handle(wdev, p);
            }
            else
            {
//                wlan_dev_report_data(wdev, p, p->tot_len);
            }
        }
        else
        {
...
//                wlan_dev_report_data(wdev, p, p->tot_len);
        ...
        {
            // forward it
            wf_u8 *frame = osZMalloc(p->tot_len);
            if (frame == OS_NULL)
            {
                LOG_E("F:%s L:%d malloc out_buf fail\n", __FUNCTION__, __LINE__);
//                pbuf_free(p);
...
//            wlan_dev_transfer_prot(wdev, p, p->tot_len);
        }
    }
    else
    {
...
int process_amsdu(wlan_dev_t *wdev, prx_pkt_t ppkt)
{
    int pktlen;
    int ret = 0;
...
static void rx_data_upload_process(rx_pkt_t *pkt, wlan_dev_t *wdev)
{
    int ret;
    if (pkt->pkt_info.amsdu)
    {
        /* amsdu process */
        ret = process_amsdu(wdev, pkt);
        if (pkt->pskb)
        {
//            pbuf_free(pkt->pskb);
        }
...
//            pbuf_free(pkt->pskb);
    ...
    {
        //LOG_W("[%s]£ºpwdn_info->ba_ctl->%p, pwdn_info->ba_ctl[%d].enable->%d",
...
void mpdu_process(wlan_dev_t *wdev, wf_u32 tot_len, wf_u32 remain_len, wf_u8 *buf, wf_u16 buf_size)
{
    wlan_priv_t *pwlan_priv = wlan_dev_priv(wdev);
...
//        pbuf_free(pkt.pskb);
    ...
    if (GET_HDR_Type(pkt.pdata) == MAC_FRAME_TYPE_DATA)
    {
        ...
        ret = create_pbuf(&pkt, pkt.pdata, pkt.len);
        if (ret)
        {
            goto exit ;
        }
        ...
        {
            rx_data_reorder_process(&pkt, wdev);
        }
        else
        {
            rx_data_upload_process(&pkt, wdev);
        }
...
int wf_rx_work(wlan_dev_t *wdev, struct hif_netbuf *netbuf)
{
    wf_u8 *pbuf;
    wf_s32 remain_len;
    wf_u16 once_len;
...
#else
        mpdu_process(wdev, netbuf->len, remain_len, pbuf, pkt_len);
#endif

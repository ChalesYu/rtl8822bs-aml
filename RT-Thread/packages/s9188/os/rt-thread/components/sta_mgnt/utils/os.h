...
/**
 * wf_os_get_reltime - Get relative time (sec, usec)
 * @t: Pointer to buffer for the time
 * Returns: 0 on success, -1 on failure
 */
...
/**
 * wf_os_get_random - Get cryptographically strong pseudo random data
...
/**
 * wf_os_memcmp_const - Constant time memory comparison
 ...
 * does not behave like os_memcmp(), i.e., wf_os_memcmp_const() cannot be used to
 * sort items into a defined order. Unlike os_memcmp(), execution time of
 * wf_os_memcmp_const() does not depend on the contents of the compared memory

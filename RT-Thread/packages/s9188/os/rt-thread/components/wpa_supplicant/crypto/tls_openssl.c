...
if (strncmp(name, "cert:
		...
		ret = CertFindCertificateInStore(cs, X509_ASN_ENCODING |
						 ...
						 wbuf, NULL);
	} else if (strncmp(name, " hash:
CRYPT_HASH_BLOB blob;
int len;
const char *hash = name + 7;
unsigned char *buf;
len = os_string_length(hash) / 2;
buf = os_malloc(len); if (buf && hex_str_to_bin(hash, buf, len) == 0) {
blob.cbData = len;
blob.pbData = buf;
ret = CertFindCertificateInStore(cs,
								...
								CertCloseStore(cs, 0); return ret;}

								static int tls_cryptoapi_cert(SSL * ssl,
															  const char *name)
								{
X509 * cert = NULL; RSA * rsa = NULL, *pub_rsa; struct cryptoapi_rsa_data *priv; RSA_METHOD * rsa_meth; if (name == NULL || (strncmp(name, "cert:
	     strncmp(name, " hash:
return -1; priv = os_zalloc(sizeof(*priv)); rsa_meth = os_zalloc(sizeof(*rsa_meth)); if (priv == NULL
|| rsa_meth == NULL) {
wpa_printf(MSG_WARNING, "CryptoAPI: Failed to allocate memory " "for CryptoAPI RSA method");
os_free(priv); os_free(rsa_meth); return -1;}
...
wpa_printf(MSG_INFO, "CryptoAPI: Could not find certificate "
		  ...
		  cert = d2i_X509(NULL,
						  ...
						  priv->cert->cbCertEncoded); if (cert == NULL) {
		  ...
		  rsa_meth->app_data = (char *)priv; rsa = RSA_new(); if (rsa == NULL) {
		  SSLerr(SSL_F_SSL_CTX_USE_CERTIFICATE_FILE,
				 ERR_R_MALLOC_FAILURE); goto err;}

		  if (!SSL_use_certificate(ssl, cert)) {
		  RSA_free(rsa); rsa = NULL; goto err;}
		  pub_rsa = cert->cert_info->key->pkey->pkey.rsa;
		  X509_free(cert);
		  cert = NULL;
		  rsa->n = BN_dup(pub_rsa->n);
		  rsa->e = BN_dup(pub_rsa->e); if (!RSA_set_method(rsa, rsa_meth))
		  goto err; if (!SSL_use_RSAPrivateKey(ssl, rsa))
goto err; RSA_free(rsa); return 0; err:
		  if (cert)
		  X509_free(cert); if (rsa)
		  RSA_free(rsa);
		  else {
		  os_free(rsa_meth); cryptoapi_free_data(priv);}
		  return -1;}

		  static int tls_cryptoapi_ca_cert(SSL_CTX * ssl_ctx, SSL * ssl,
										   const char *name) {
		  HCERTSTORE cs; PCCERT_CONTEXT ctx = NULL; X509 * cert; char buf[128];
		  const char *store;
#ifdef UNICODE
		  WCHAR * wstore;
#endif
if (name == NULL || strncmp(name, "cert_store:
		return -1;

	store = name + 13;
#ifdef UNICODE
	wstore = os_malloc((os_string_length(store) + 1) * sizeof(WCHAR));
	if (wstore == NULL)
		return -1;
					  ...
					  const char *value, size_t len)
{
	...
	for (i = 0; ext && i < sk_GENERAL_NAME_num(ext); i++) {
		...
		if (gen->type != type)
			continue;
		if (os_string_length((char *) gen->d.ia5->data) == len &&
		    os_memcompare(value, gen->d.ia5->data, len) == 0)
			found++;
	}
			       ...
			       int full)
{
	...
	match_len = os_string_length(match);
				   ...
				   const char *subject)
{
	...
	if (cert) {
		...
		if (sha256_vect(1, addr, len, hash) == 0) {
...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	if (depth == 0 && conn->server_cert_only) {
		...
		if (!cert) {
			...
			len[0] = wpabuf_len(cert);
			if (sha256_vect(1, addr, len, hash) < 0 ||
			    os_memcompare(conn->srv_cert_hash, hash, 32) != 0) {
				  ...
				  size_t ca_cert_blob_len, const char *ca_path)
{
	...
	if (ca_cert && os_strncmp(ca_cert, " probe:
wpa_printf(MSG_DEBUG, "OpenSSL: Probe for server certificate "
"chain"); conn->cert_probe = 1; conn->ca_cert_verify = 0; return 0;}

if (ca_cert && os_strncmp(ca_cert, "hash:
#ifdef CONFIG_SHA256
...
#ifdef ANDROID
	if (ca_cert && os_strncmp(" keystore:
BIO * bio = BIO_from_keystore(&ca_cert[11]); STACK_OF(X509_INFO) * stack = NULL; stack_index_t i; if (bio) {
stack = PEM_X509_INFO_read_bio(bio, NULL, NULL, NULL); BIO_free(bio);}

if (!stack)
return -1; for (i = 0; i < sk_X509_INFO_num(stack); ++i) {
X509_INFO * info = sk_X509_INFO_value(stack, i); if (info->x509) {
X509_STORE_add_cert(ssl_ctx->cert_store, info->x509);}
if (info->crl) {
X509_STORE_add_crl(ssl_ctx->cert_store, info->crl);}
}
sk_X509_INFO_pop_free(stack, X509_INFO_free);
SSL_set_verify(conn->ssl, SSL_VERIFY_PEER, tls_verify_cb); return 0;}
#endif

#ifdef CONFIG_NATIVE_WINDOWS
if (ca_cert && tls_cryptoapi_ca_cert(ssl_ctx, conn->ssl, ca_cert) == 0) {
wpa_printf(MSG_DEBUG, "OpenSSL: Added CA certificates from "
		  "system certificate store"); return 0;}
#endif
		  ...
		  wpa_printf(MSG_DEBUG, "OpenSSL: %s - loaded "
					 "DER format CA certificate", __func__);}
		  else
		  return -1;}
		  else {
		  wpa_printf(MSG_DEBUG, "TLS: Trusted root "
					 "certificate(s) loaded"); tls_get_errors(data);}
#else
		  ...
		  static int tls_global_ca_cert(struct tls_data *data,
										const char *ca_cert) {
		  SSL_CTX * ssl_ctx = data->ssl; if (ca_cert) {
		  if (SSL_CTX_load_verify_locations(ssl_ctx, ca_cert, NULL) != 1) {
		  tls_show_errors(MSG_WARNING, __func__,
						  "Failed to load root certificates"); return -1;}
		  ...
		  int flags; if (check_crl) {
		  struct tls_data *data = ssl_ctx;
		  X509_STORE * cs = SSL_CTX_get_cert_store(data->ssl); if (cs == NULL) {
		  tls_show_errors(MSG_INFO, __func__, "Failed to get "
						  "certificate store when enabling "
						  "check_crl"); return -1;}
		  flags = X509_V_FLAG_CRL_CHECK; if (check_crl == 2)
		  flags |= X509_V_FLAG_CRL_CHECK_ALL; X509_STORE_set_flags(cs, flags);}
		  return 0;}
		  ...
		  {
		  os_free(conn->subject_match); conn->subject_match = NULL;
		  if (subject_match) {
		  ...
		  os_free(conn->altsubject_match);
		  conn->altsubject_match = NULL; if (altsubject_match) {
		  conn->altsubject_match = os_strdup(altsubject_match);
		  if (conn->altsubject_match == NULL)
		  return -1;}

		  os_free(conn->suffix_match);
		  conn->suffix_match = NULL; if (suffix_match) {
		  conn->suffix_match = os_strdup(suffix_match);
		  if (conn->suffix_match == NULL)
		  return -1;}

		  os_free(conn->domain_match);
		  conn->domain_match = NULL; if (domain_match) {
		  ...
		  int tls_connection_set_verify(void *ssl_ctx,
										struct tls_connection *conn,
										int verify_peer, unsigned int flags,
										const u8 * session_ctx,
										size_t session_ctx_len) {
		  static int counter = 0; struct tls_data *data = ssl_ctx;
		  if (conn == NULL)
		  return -1; if (verify_peer) {
		  conn->ca_cert_verify = 1;
			    ...
			    const char *passwd)
{
	...
	if (certs) {
		...
		if (!res) {
			...
			wpa_printf(MSG_DEBUG,
				   " TLS:Ignore certificate chain verification error when building chain with
						 PKCS
#12 extra certificates");
						 }
#endif
						 res = 0;
#else
						 ...
						 PKCS12_free(p12); if (res < 0)
						 tls_get_errors(data); return res;}
#endif

						 static int tls_read_pkcs12(struct tls_data *data,
													SSL * ssl,
						 ...
						 FILE * f;
						 PKCS12 * p12;
						 f = fopen(private_key, "rb"); if (f == NULL)
						 return -1;
						 p12 = d2i_PKCS12_fp(f, NULL);
						 fclose(f); if (p12 == NULL) {
						 tls_show_errors(MSG_INFO, __func__,
						 ...
						 wpa_printf(MSG_INFO,
									...
									"p12/pfx files"); return -1;
#endif
						 }

						 static int tls_read_pkcs12_blob(struct tls_data *data,
														 SSL * ssl,
														 const u8 * blob,
														 size_t len,
														 const char *passwd) {
						 ...
						 PKCS12 * p12;
						 p12 =
						 d2i_PKCS12(NULL, (const unsigned char **)&blob, len);
						 if (p12 == NULL) {
						 tls_show_errors(MSG_INFO, __func__,
						 ...
						 wpa_printf(MSG_ERROR,
									...
									" '%s' [%s]", cert_id, ERR_error_string(err,
																			NULL));
						 if (tls_is_pin_error(err))
						 ...
						 wpa_printf(MSG_ERROR,
									...
									" '%s'", cert_id);
						 return TLS_SET_PARAMS_ENGINE_PRV_INIT_FAILED;}
						 *cert = params.cert; return 0;}
#endif

						 static int tls_connection_engine_client_cert(struct
																	  tls_connection
																	  *conn,
																	  const char
																	  *cert_id)
						 ...
						 return -1; if (!SSL_use_certificate(conn->ssl, cert)) {
						 ...
						 static int tls_connection_engine_ca_cert(struct
																  tls_data
																  *data,
																  struct
																  tls_connection
						 ...
						 return -1; store = X509_STORE_new(); if (store == NULL) {
						 wpa_printf(MSG_DEBUG,
						 ...
						 wpa_printf(MSG_DEBUG,
									...
									" already in hash table error", __func__);}
						 else {
						 X509_free(cert); return -1;}
						 }
						 X509_free(cert);
						 ...
						 SSL_set_verify(conn->ssl, SSL_VERIFY_PEER,
										tls_verify_cb);
						 conn->ca_cert_verify = 1; return 0;
#else
						 return -1;
#endif
						 }

						 static int tls_connection_engine_private_key(struct
																	  tls_connection
																	  *conn) {
#ifndef OPENSSL_NO_ENGINE
						 if (SSL_use_PrivateKey(conn->ssl, conn->private_key) !=
							 1) {
						 tls_show_errors(MSG_ERROR, __func__,
						 ...
						 static int tls_connection_private_key(struct tls_data
															   *data,
															   struct
															   tls_connection
															   *conn,
						 ...
						 {
						 SSL_CTX * ssl_ctx = data->ssl; char *passwd; int ok;
						 if (private_key == NULL && private_key_blob == NULL)
						 return 0; if (private_key_passwd) {
						 ...
						 if (SSL_use_PrivateKey_ASN1
							 (EVP_PKEY_RSA, conn->ssl, (u8 *) private_key_blob,
							  private_key_blob_len) == 1) {
						 wpa_printf(MSG_DEBUG,
									"OpenSSL: SSL_use_PrivateKey_"
									"ASN1(EVP_PKEY_RSA) --> OK"); ok = 1;
						 break;}
						 ...
						 if (tls_read_pkcs12_blob
							 (data, conn->ssl, private_key_blob,
							  private_key_blob_len, passwd) == 0) {
						 wpa_printf(MSG_DEBUG,
									"OpenSSL: PKCS#12 as blob --> " "OK");
						 ok = 1; break;}
...
#endif
						 if (tls_read_pkcs12
							 (data, conn->ssl, private_key, passwd)
							 == 0) {
						 ...
						 static int tls_global_private_key(struct tls_data
														   *data,
														   const char
														   *private_key,
														   const char
														   *private_key_passwd)
						 {
						 SSL_CTX * ssl_ctx = data->ssl; char *passwd;
						 if (private_key == NULL)
						 return 0; if (private_key_passwd) {
						 passwd = os_strdup(private_key_passwd);
						 if (passwd == NULL)
						 ...
						 tls_show_errors(MSG_INFO, __func__,
										 "Failed to load private key");
						 os_free(passwd); ERR_clear_error(); return -1;}
						 os_free(passwd); ERR_clear_error();
						 SSL_CTX_set_default_passwd_cb(ssl_ctx, NULL);
						 ...
						 wpa_printf(MSG_ERROR,
									...
									"dh_file specified"); return -1;
#else
						 DH * dh; BIO * bio; if (dh_file == NULL)
						 return 0; if (conn == NULL)
						 return -1;
						 bio = BIO_new_file(dh_file, "r"); if (bio == NULL) {
						 wpa_printf(MSG_INFO,
									"TLS: Failed to open DH file '%s': %s",
									dh_file, ERR_error_string(ERR_get_error(),
															  NULL));
						 return -1;}
						 ...
						 BIO_free(bio); if (!dsa) {
						 wpa_printf(MSG_DEBUG,
									"TLS: Failed to parse DSA file " "'%s': %s",
									dh_file, ERR_error_string(ERR_get_error(),
															  NULL)); break;}

						 wpa_printf(MSG_DEBUG,
									"TLS: DH file in DSA param format");
						 dh = DSA_dup_DH(dsa); DSA_free(dsa); if (dh == NULL) {
						 wpa_printf(MSG_INFO,
									"TLS: Failed to convert DSA "
									"params into DH params"); break;}
						 break;}
#endif
						 if (dh == NULL) {
						 wpa_printf(MSG_INFO,
						 ...
						 static int tls_global_dh(struct tls_data *data,
												  const char *dh_file) {
#ifdef OPENSSL_NO_DH
						 if (dh_file == NULL)
						 return 0;
						 ...
						 int tls_connection_get_random(void *ssl_ctx,
													   struct tls_connection
													   *conn,
													   struct tls_random *keys)
						 {
						 SSL * ssl; if (conn == NULL || keys == NULL)
						 return -1; ssl = conn->ssl;
#if OPENSSL_VERSION_NUMBER < 0x10100000L
						 if (ssl == NULL || ssl->s3 == NULL
							 || ssl->session == NULL)
						 return -1; os_memset(keys, 0, sizeof(*keys));
						 ...
						 SSL_get_client_random(ssl, conn->client_random,
											   sizeof(conn->client_random));
						 keys->server_random = conn->server_random;
						 keys->server_random_len =
						 SSL_get_server_random(ssl, conn->server_random,
						 ...
						 const EVP_MD * h;
						 int md_size;
						 if (ssl->enc_read_ctx == NULL
							 || ssl->enc_read_ctx->cipher == NULL
							 || ssl->read_hash == NULL)
						 return -1; c = ssl->enc_read_ctx->cipher;
						 ...
						 wpa_printf(MSG_DEBUG,
									...
									"IV_len=%d", EVP_CIPHER_key_length(c),
									md_size, EVP_CIPHER_iv_length(c));
						 return 2 * (EVP_CIPHER_key_length(c) + md_size +
						 ...
						 return -1; c = EVP_get_cipherbynid(cipher);
						 h = EVP_get_digestbynid(digest); if (!c || !h)
						 return -1;
						 wpa_printf(MSG_DEBUG,
						 ...
						 int skip = 0;
						 u8 * tmp_out = NULL;
						 u8 * _out = out; const char *ver; if (conn == NULL)
						 return -1;
						 ssl = conn->ssl;
						 if (ssl == NULL || ssl->s3 == NULL
							 || ssl->session == NULL
							 || ssl->session->master_key_length <= 0)
						 return -1; ver = SSL_get_version(ssl);
						 if (skip_keyblock) {
						 ...
						 rnd = os_malloc(2 * SSL3_RANDOM_SIZE); if (!rnd) {
						 os_free(tmp_out); return -1;}

						 if (server_random_first) {
						 os_memcpy(rnd, ssl->s3->server_random,
								   SSL3_RANDOM_SIZE);
						 os_memcpy(rnd + SSL3_RANDOM_SIZE,
								   ssl->s3->client_random, SSL3_RANDOM_SIZE);}
						 else {
						 os_memcpy(rnd, ssl->s3->client_random,
								   SSL3_RANDOM_SIZE);
						 os_memcpy(rnd + SSL3_RANDOM_SIZE,
						 ...
						 if (tls_prf_sha1_md5(ssl->session->master_key,
											  ...
											  _out, skip + out_len) == 0) {
						 ret = 0;}
						 os_free(rnd); if (ret == 0 && skip_keyblock)
						 os_memcpy(out, _out + skip, out_len);
						 bin_clear_free(tmp_out, skip); return ret;
#else
						 SSL * ssl;
						 SSL_SESSION * sess;
						 u8 * rnd;
						 ...
						 const char *ver; if (conn == NULL)
						 return -1; ssl = conn->ssl; if (ssl == NULL)
						 return -1;
						 ver = SSL_get_version(ssl);
						 sess = SSL_get_session(ssl); if (!ver || !sess)
						 return -1; if (skip_keyblock) {
						 skip = openssl_get_keyblock_size(ssl); if (skip < 0)
						 return -1;
						 tmp_out = os_malloc(skip + out_len); if (!tmp_out)
						 return -1; _out = tmp_out;}
						 ...
						 SSL_get_client_random(ssl, client_random,
											   sizeof(client_random));
						 SSL_get_server_random(ssl, server_random,
											   sizeof(server_random));
						 master_key_len =
						 SSL_SESSION_get_master_key(sess, master_key,
													sizeof(master_key));
						 if (server_random_first) {
						 ...
						 int tls_connection_prf(void *tls_ctx,
												...
												size_t out_len) {
#if OPENSSL_VERSION_NUMBER >= 0x10001000L
						 SSL * ssl; if (conn == NULL)
						 return -1; if (server_random_first || skip_keyblock)
						 return openssl_tls_prf(conn, label,
												server_random_first,
												skip_keyblock, out, out_len);
						 ssl = conn->ssl;
						 if (SSL_export_keying_material
							 (ssl, out, out_len, label, os_string_length(label),
							  NULL, 0, 0) == 1) {
						 ...
						 static struct wpabuf *openssl_handshake(struct
																 tls_connection
																 *conn,
																 const struct
																 wpabuf
																 *in_data,
																 int server) {
						 int res; struct wpabuf *out_data;
						 if (in_data && wpabuf_len(in_data) > 0
							 && BIO_write(conn->ssl_in, wpabuf_head(in_data),
										  wpabuf_len(in_data))
							 < 0) {
						 tls_show_errors(MSG_INFO, __func__,
						 ...
						 res = SSL_connect(conn->ssl); if (res != 1) {
						 ...
						 out_data = wpabuf_alloc(res); if (out_data == NULL) {
						 wpa_printf(MSG_DEBUG,
						 ...
						 return NULL;}
						 res =
						 res == 0 ? 0 : BIO_read(conn->ssl_out,
												 wpabuf_mhead(out_data), res);
						 if (res < 0) {
						 tls_show_errors(MSG_INFO, __func__,
						 ...
						 tls_show_errors(MSG_INFO, __func__,
										 "BIO_reset failed");}
						 wpabuf_free(out_data); return NULL;}
						 wpabuf_put(out_data, res); return out_data;}

						 static struct wpabuf *openssl_get_appl_data(struct
																	 tls_connection
																	 *conn,
																	 size_t
																	 max_len) {
						 struct wpabuf *appl_data; int res;
						 appl_data = wpabuf_alloc(max_len + 100);
						 if (appl_data == NULL)
						 return NULL;
						 res =
						 SSL_read(conn->ssl, wpabuf_mhead(appl_data),
								  wpabuf_size(appl_data)); if (res < 0) {
						 int err = SSL_get_error(conn->ssl, res);
						 if (err == SSL_ERROR_WANT_READ
						 ...
						 tls_show_errors(MSG_INFO, __func__,
										 ...
										 "Application Data");}
						 wpabuf_free(appl_data); return NULL;}
						 ...
						 *openssl_connection_handshake(struct tls_connection
													   ...
													   **appl_data,
													   int server) {
						 struct wpabuf *out_data; if (appl_data)
						 * appl_data = NULL;
						 out_data = openssl_handshake(conn, in_data, server);
						 if (out_data == NULL)
						 return NULL; if (conn->invalid_hb_used) {
						 wpa_printf(MSG_INFO,
						 ...
						 wpa_printf(MSG_DEBUG,
									...
									tls_connection_resumed(conn->ssl_ctx,
														   conn));
						 if (appl_data && in_data)
						 * appl_data =
						 openssl_get_appl_data(conn, wpabuf_len(in_data));}

						 if (conn->invalid_hb_used) {
						 wpa_printf(MSG_INFO,
									"TLS: Heartbeat attack detected - do not send response");
						 if (appl_data) {
						 wpabuf_free(*appl_data); *appl_data = NULL;}
						 wpabuf_free(out_data); return NULL;}
						 ...
						 struct wpabuf *tls_connection_handshake(void *ssl_ctx,
																 ...
																 *in_data,
																 struct wpabuf
																 **appl_data) {
						 return openssl_connection_handshake(conn, in_data,
															 appl_data, 0);}

						 struct wpabuf *tls_connection_server_handshake(void
																		*tls_ctx,
																		struct
																		tls_connection
																		*conn,
						 ...
						 struct wpabuf *tls_connection_encrypt(void *tls_ctx,
															   ...
															   *conn,
															   ...
															   wpabuf *in_data)
						 {
						 int res; struct wpabuf *buf; if (conn == NULL)
						 return NULL;
						 if ((res = BIO_reset(conn->ssl_in)) < 0 ||
							 (res = BIO_reset(conn->ssl_out)) < 0) {
						 tls_show_errors(MSG_INFO, __func__,
										 "BIO_reset failed"); return NULL;}
						 res =
						 SSL_write(conn->ssl, wpabuf_head(in_data),
								   wpabuf_len(in_data)); if (res < 0) {
						 tls_show_errors(MSG_INFO, __func__,
						 ...
						 return NULL;
						 res =
						 BIO_read(conn->ssl_out, wpabuf_mhead(buf),
								  wpabuf_size(buf)); if (res < 0) {
						 tls_show_errors(MSG_INFO, __func__,
										 "Encryption failed - BIO_read");
						 wpabuf_free(buf); return NULL;}
						 wpabuf_put(buf, res); return buf;}
						 ...
						 return NULL;
						 res =
						 SSL_read(conn->ssl, wpabuf_mhead(buf),
								  wpabuf_size(buf)); if (res < 0) {
						 tls_show_errors(MSG_INFO, __func__,
										 "Decryption failed - SSL_read");
						 wpabuf_free(buf); return NULL;}
						 wpabuf_put(buf, res); if (conn->invalid_hb_used) {
						 wpa_printf(MSG_INFO,
									"TLS: Heartbeat attack detected - do not send response");
						 wpabuf_free(buf); return NULL;}
						 ...
						 int tls_connection_resumed(void *ssl_ctx,
													struct tls_connection *conn)
						 {
#if OPENSSL_VERSION_NUMBER >= 0x10001000L
						 ...
						 int tls_connection_set_cipher_list(void *tls_ctx,
															...
															*conn,
															u8 * ciphers) {
						 char buf[100], *pos, *end; u8 * c; int ret;
						 if (conn == NULL || conn->ssl == NULL
							 || ciphers == NULL)
						 return -1; buf[0] = '\0'; pos = buf;
						 end = pos + sizeof(buf); c = ciphers;
						 while (*c != TLS_CIPHER_NONE) {
						 const char *suite; switch (*c) {
case TLS_CIPHER_RC4_SHA:
suite = "RC4-SHA"; break; case TLS_CIPHER_AES128_SHA:
suite = "AES128-SHA"; break; case TLS_CIPHER_RSA_DHE_AES128_SHA:
suite = "DHE-RSA-AES128-SHA"; break; case TLS_CIPHER_ANON_DH_AES128_SHA:
suite = "ADH-AES128-SHA"; break; default:
						 wpa_printf(MSG_DEBUG, "TLS: Unsupported "
									"cipher selection: %d", *c); return -1;}
						 ret = os_snprintf(pos, end - pos, ":%s", suite);
						 if (os_snprintf_error(end - pos, ret))
						 break; pos += ret; c++;}

						 wpa_printf(MSG_DEBUG, "OpenSSL: cipher suites: %s",
									buf + 1);
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
						 ...
						 if (SSL_get_security_level(conn->ssl) == 0) {
						 ...
						 int tls_get_version(void *ssl_ctx,
											 struct tls_connection *conn,
											 char *buf, size_t buflen) {
						 const char *name;
						 if (conn == NULL || conn->ssl == NULL)
						 return -1; name = SSL_get_version(conn->ssl);
						 if (name == NULL)
						 return -1; os_strlcpy(buf, name, buflen); return 0;}

						 int tls_get_cipher(void *ssl_ctx,
											struct tls_connection *conn,
											char *buf, size_t buflen) {
						 const char *name;
						 if (conn == NULL || conn->ssl == NULL)
						 return -1; name = SSL_get_cipher(conn->ssl);
						 if (name == NULL)
						 return -1; os_strlcpy(buf, name, buflen); return 0;}
...
#if defined(EAP_FAST) || defined(EAP_FAST_DYNAMIC) || defined(EAP_SERVER_FAST)
						 int tls_connection_client_hello_ext(void *ssl_ctx,
															 struct
															 tls_connection
															 *conn,
															 int ext_type,
						 ...
						 if (conn == NULL || conn->ssl == NULL
							 || ext_type != 35)
						 return -1;
						 if (SSL_set_session_ticket_ext
							 (conn->ssl, (void *)data, data_len) != 1)
						 return -1; return 0;}
#endif

						 int tls_connection_get_failed(void *ssl_ctx,
													   struct tls_connection
													   *conn) {
						 if (conn == NULL)
						 return -1; return conn->failed;}

						 int tls_connection_get_read_alerts(void *ssl_ctx,
															struct
															tls_connection
															*conn) {
						 if (conn == NULL)
						 return -1; return conn->read_alerts;}
						 ...
						 int tls_connection_get_write_alerts(void *ssl_ctx,
															 ...
															 *conn) {
						 if (conn == NULL)
						 return -1; return conn->write_alerts;}
						 ...
						 BIO * out;
						 size_t rlen;
						 char *txt; int res; if (wpa_debug_level > MSG_DEBUG)
						 return; out = BIO_new(BIO_s_mem()); if (!out)
						 return;
						 OCSP_RESPONSE_print(out, rsp, 0);
						 rlen = BIO_ctrl_pending(out);
						 txt = os_malloc(rlen + 1); if (!txt) {
						 BIO_free(out); return;}

						 res = BIO_read(out, txt, rlen); if (res > 0) {
						 txt[res] = '\0';
						 wpa_printf(MSG_DEBUG, "OpenSSL: OCSP Response\n%s",
									txt);}
						 os_free(txt); BIO_free(out);
#endif
						 }

						 static void debug_print_cert(X509 * cert,
													  const char *title) {
#ifndef CONFIG_NO_STDOUT_DEBUG
						 BIO * out;
						 size_t rlen;
						 ...
						 *next_update; X509_STORE * store;
						 STACK_OF(X509) * certs = NULL;
						 len = SSL_get_tlsext_status_ocsp_resp(s, &p); if (!p) {
						 wpa_printf(MSG_DEBUG,
									"OpenSSL: No OCSP response received");
						 return (conn->flags & TLS_CONN_REQUIRE_OCSP) ? 0 : 1;}

						 wpa_hexdump(MSG_DEBUG, "OpenSSL: OCSP response", p,
									 len);
						 rsp = d2i_OCSP_RESPONSE(NULL, &p, len); if (!rsp) {
						 wpa_printf(MSG_INFO,
						 ...
						 basic = OCSP_response_get1_basic(rsp); if (!basic) {
						 wpa_printf(MSG_INFO,
						 ...
						 if (X509_STORE_add_cert(store, conn->peer_issuer) !=
							 1) {
						 tls_show_errors(MSG_INFO, __func__,
										 "OpenSSL: Could not add issuer to certificate store");}
						 certs = sk_X509_new_null(); if (certs) {
						 X509 * cert;
						 cert = X509_dup(conn->peer_issuer);
						 if (cert && !sk_X509_push(certs, cert)) {
						 tls_show_errors(MSG_INFO, __func__,
										 "OpenSSL: Could not add issuer to OCSP responder trust store");
						 X509_free(cert); sk_X509_free(certs); certs = NULL;}
						 ...
						 status =
						 OCSP_basic_verify(basic, certs, store,
										   OCSP_TRUSTOTHER);
						 sk_X509_pop_free(certs, X509_free); if (status <= 0) {
						 tls_show_errors(MSG_INFO, __func__,
										 "OpenSSL: OCSP response failed verification");
						 OCSP_BASICRESP_free(basic); OCSP_RESPONSE_free(rsp);
						 return 0;}
						 ...
						 id =
						 OCSP_cert_to_id(NULL, conn->peer_cert,
										 conn->peer_issuer); if (!id) {
						 wpa_printf(MSG_DEBUG,
						 ...
						 return 1; if (status == V_OCSP_CERTSTATUS_REVOKED)
						 return 0; if (conn->flags & TLS_CONN_REQUIRE_OCSP) {
						 wpa_printf(MSG_DEBUG,
						 ...
						 resp =
						 os_readfile(tls_global->ocsp_stapling_response, &len);
						 if (resp == NULL) {
						 wpa_printf(MSG_DEBUG,
						 ...
						 tmp = OPENSSL_malloc(len); if (tmp == NULL) {
						 os_free(resp); return SSL_TLSEXT_ERR_ALERT_FATAL;}
						 ...
						 if (SSL_set_ssl_method(conn->ssl, TLSv1_method()) !=
							 1) {
						 tls_show_errors(MSG_INFO, __func__,
						 ...
						 if (tls_connection_set_subject_match(conn,
															  ...
															  params->
															  domain_match))
						 return -1; if (engine_id && ca_cert_id) {
						 if (tls_connection_engine_ca_cert
							 (data, conn, ca_cert_id))
						 ...
						 if (tls_connection_ca_cert(data, conn, params->ca_cert,
													...
													params->ca_path))
						 return -1; if (engine_id && cert_id) {
						 if (tls_connection_engine_client_cert(conn, cert_id))
						 return TLS_SET_PARAMS_ENGINE_PRV_VERIFY_FAILED;}
						 else
						 if (tls_connection_client_cert
							 (conn, params->client_cert,
							  params->client_cert_blob,
							  params->client_cert_blob_len))
						 return -1; if (engine_id && key_id) {
						 wpa_printf(MSG_DEBUG,
									"TLS: Using private key from engine");
						 if (tls_connection_engine_private_key(conn))
						 return TLS_SET_PARAMS_ENGINE_PRV_VERIFY_FAILED;}
						 else
						 if (tls_connection_private_key(data, conn,
														params->private_key,
														params->
														private_key_passwd,
														params->
						 ...
						 wpa_printf(MSG_INFO,
									...
									params->private_key); return -1;}

						 if (tls_connection_dh(conn, params->dh_file)) {
						 wpa_printf(MSG_INFO,
									"TLS: Failed to load DH file '%s'",
									params->dh_file); return -1;}

						 if (params->openssl_ciphers &&
							 SSL_set_cipher_list(conn->ssl,
												 params->openssl_ciphers) !=
							 1) {
						 wpa_printf(MSG_INFO,
									"OpenSSL: Failed to set cipher string '%s'",
									params->openssl_ciphers); return -1;}

						 tls_set_conn_flags(conn->ssl, params->flags);
#ifdef HAVE_OCSP
						 if (params->flags & TLS_CONN_REQUEST_OCSP) {
						 SSL_CTX * ssl_ctx = data->ssl;
						 SSL_set_tlsext_status_type(conn->ssl,
						 ...
						 SSL_CTX * ssl_ctx = data->ssl; unsigned long err;
						 while ((err = ERR_get_error())) {
						 wpa_printf(MSG_INFO,
									"%s: Clearing pending SSL error: %s",
									__func__, ERR_error_string(err, NULL));}
						 ...
						 static int tls_sess_sec_cb(SSL * s, void *secret,
													int *secret_len,
													STACK_OF(SSL_CIPHER) *
													peer_ciphers,
...
#endif
						 {
						 struct tls_connection *conn = arg; int ret;
#if OPENSSL_VERSION_NUMBER < 0x10100000L
						 ...
						 if (conn == NULL || conn->session_ticket_cb == NULL)
						 return 0;
						 SSL_get_client_random(s, client_random,
											   sizeof(client_random));
						 SSL_get_server_random(s, server_random,
											   sizeof(server_random));
						 ret =
						 conn->session_ticket_cb(conn->session_ticket_cb_ctx,
												 conn->session_ticket,
...
#endif
						 os_free(conn->session_ticket);
						 conn->session_ticket = NULL; if (ret <= 0)
						 return 0;
						 *secret_len = SSL_MAX_MASTER_KEY_LENGTH; return 1;}

						 static int tls_session_ticket_ext_cb(SSL * s,
															  const unsigned
															  char *data,
															  int len,
															  void *arg) {
						 ...
						 if (conn == NULL || conn->session_ticket_cb == NULL)
						 return 0;
						 wpa_printf(MSG_DEBUG, "OpenSSL: %s: length=%d",
									__func__, len);
						 os_free(conn->session_ticket);
						 ...
						 return 0; os_memcpy(conn->session_ticket, data, len);
						 conn->session_ticket_len = len; return 1;}
#endif
						 ...
						 conn->session_ticket_cb_ctx = ctx; if (cb) {
						 if (SSL_set_session_secret_cb
							 (conn->ssl, tls_sess_sec_cb, conn) != 1)
						 return -1;
						 SSL_set_session_ticket_ext_cb(conn->ssl,
													   tls_session_ticket_ext_cb,
													   conn);}
						 else {
						 if (SSL_set_session_secret_cb(conn->ssl, NULL, NULL) !=
							 1)
						 return -1;
						 ...
						 void tls_connection_set_success_data(struct
															  tls_connection
															  *conn,
															  struct wpabuf
															  *data) {
						 SSL_SESSION * sess; struct wpabuf *old;
						 if (tls_ex_idx_session < 0)
						 goto fail; sess = SSL_get_session(conn->ssl);
						 if (!sess)
						 goto fail;
						 old =
						 SSL_SESSION_get_ex_data(sess, tls_ex_idx_session);
						 if (old) {
						 wpa_printf(MSG_DEBUG,
									"OpenSSL: Replacing old success data %p",
									old); wpabuf_free(old);}
						 if (SSL_SESSION_set_ex_data
							 (sess, tls_ex_idx_session, data) != 1)
goto fail; wpa_printf(MSG_DEBUG, "OpenSSL: Stored success data %p", data); conn->success_data = 1; return; fail:
						 wpa_printf(MSG_INFO,
									"OpenSSL: Failed to store success data");
						 wpabuf_free(data);}

						 void tls_connection_set_success_data_resumed(struct
																	  tls_connection
																	  *conn) {
						 wpa_printf(MSG_DEBUG,
						 ...
						 *tls_connection_get_success_data(struct tls_connection
						 ...
						 return NULL;
						 return SSL_SESSION_get_ex_data(sess,
														tls_ex_idx_session);}

						 void tls_connection_remove_session(struct
															tls_connection
															*conn) {
						 SSL_SESSION * sess; sess = SSL_get_session(conn->ssl);
						 if (!sess)
						 return;

...
static ssize_t tls_pull_func(gnutls_transport_ptr_t ptr, void *buf, size_t len)
{
	...
	if (conn->pull_buf_offset == end) {
		...
		wpa_printf(MSG_DEBUG, "%s - %lu bytes remaining in pull_buf",
				   __func__, (unsigned long)(end - conn->pull_buf_offset));
	}
	return len;
}
...
struct tls_connection *tls_connection_init(void *ssl_ctx)
{
	...
	if (global->params_set) {
		ret = gnutls_credentials_set(conn->session,
									 GNUTLS_CRD_CERTIFICATE, global->xcred);
		if (ret < 0) {
							  ...
							  const struct tls_connection_params *params)
{
	...
	if (params->ca_cert) {
		wpa_printf(MSG_DEBUG, "GnuTLS: Try to parse %s in DER format",
				   params->ca_cert);
		ret =
			gnutls_certificate_set_x509_trust_file(conn->xcred, params->ca_cert,
												   GNUTLS_X509_FMT_DER);
		if (ret < 0) {
			...
			ret =
				gnutls_certificate_set_x509_trust_file(conn->xcred,
													   params->ca_cert,
													   GNUTLS_X509_FMT_PEM);
			if (ret < 0) {
				wpa_printf(MSG_DEBUG,
		...
		ret =
			gnutls_certificate_set_x509_trust_mem(conn->xcred, &ca,
												  GNUTLS_X509_FMT_DER);
		if (ret < 0) {
			wpa_printf(MSG_DEBUG,
	...
	if (params->client_cert && params->private_key) {
		...
		ret =
			gnutls_certificate_set_x509_key_file2(conn->xcred,
												  params->client_cert,
												  params->private_key,
												  GNUTLS_X509_FMT_DER,
...
#else
		ret =
			gnutls_certificate_set_x509_key_file(conn->xcred,
												 params->client_cert,
												 params->private_key,
												 GNUTLS_X509_FMT_DER);
#endif
		if (ret < 0) {
			...
			if (ret < 0) {
				wpa_printf(MSG_DEBUG, "Failed to read client "
						   "cert/key in PEM format: %s", gnutls_strerror(ret));
				return ret;
			}
		}
...
#ifdef PKCS12_FUNCS
		ret =
			gnutls_certificate_set_x509_simple_pkcs12_file(conn->xcred,
														   params->private_key,
														   GNUTLS_X509_FMT_DER,
														   params->
														   private_key_passwd);
...
#if GNUTLS_VERSION_NUMBER >= 0x03010b
		ret =
			gnutls_certificate_set_x509_key_mem2(conn->xcred, &cert, &key,
												 GNUTLS_X509_FMT_DER,
												 params->private_key_passwd, 0);
#else
		ret =
			gnutls_certificate_set_x509_key_mem(conn->xcred, &cert, &key,
												GNUTLS_X509_FMT_DER);
#endif
		if (ret < 0) {
			wpa_printf(MSG_DEBUG, "Failed to read client cert/key "
		...
		ret =
			gnutls_certificate_set_x509_simple_pkcs12_mem(conn->xcred, &key,
														  GNUTLS_X509_FMT_DER,
														  params->
														  private_key_passwd);
		if (ret != 0) {
						  ...
						  const struct tls_connection_params *params)
{
	...
	if (params->ca_cert) {
		ret =
			gnutls_certificate_set_x509_trust_file(global->xcred,
												   params->ca_cert,
												   GNUTLS_X509_FMT_DER);
		if (ret < 0) {
	...
	if (params->client_cert && params->private_key) {
		ret =
			gnutls_certificate_set_x509_key_file(global->xcred,
												 params->client_cert,
												 params->private_key,
												 GNUTLS_X509_FMT_DER);
		if (ret < 0) {
...
#ifdef PKCS12_FUNCS
		ret =
			gnutls_certificate_set_x509_simple_pkcs12_file(global->xcred,
														   params->private_key,
														   GNUTLS_X509_FMT_DER,
														   params->
														   private_key_passwd);
					   ...
					   int skip_keyblock, u8 * out, size_t out_len)
{
	...
	return gnutls_prf(conn->session, os_string_length(label), label,
...
ocsp_error:
	gnutls_tls_fail_event(conn, NULL, 0, NULL,
						  "bad certificate status response", TLS_FAIL_REVOKED);
...
static int tls_connection_verify_peer(gnutls_session_t session)
{
	...
	if (conn->verify_peer && (status & GNUTLS_CERT_INVALID)) {
		...
		if (status & GNUTLS_CERT_EXPIRED) {
			...
			gnutls_tls_fail_event(conn, NULL, 0, NULL,
								  "certificate has expired", TLS_FAIL_EXPIRED);
			err = GNUTLS_A_CERTIFICATE_EXPIRED;
			goto out;
		}
		gnutls_tls_fail_event(conn, NULL, 0, NULL,
							  "untrusted certificate", TLS_FAIL_UNTRUSTED);
		err = GNUTLS_A_INTERNAL_ERROR;
		goto out;
	}
	...
	for (i = 0; i < num_certs; i++) {
		...
		if (conn->global->event_cb) {
			...
			_len[0] = certs[i].size;
			if (sha256_vect(1, _addr, _len, hash) == 0) {
		...
		if (i == 0) {
			...
			if (conn->domain_match &&
				!gnutls_x509_crt_check_hostname2(cert, conn->domain_match,
		...
		if (!conn->disable_time_checks &&
			...
			wpa_printf(MSG_INFO, "TLS: Peer certificate %d/%d is "
					   "not valid at this time", i + 1, num_certs);
			gnutls_tls_fail_event(conn, &certs[i], i, buf,

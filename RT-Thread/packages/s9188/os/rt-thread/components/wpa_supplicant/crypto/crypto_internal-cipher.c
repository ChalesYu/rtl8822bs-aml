										 ...
										 size_t key_len)
{
	...
	case CRYPTO_CIPHER_ALG_AES:
		ctx->u.aes.ctx_enc = _init_aes_encrypt(key, key_len);
		...
		if (ctx->u.aes.ctx_dec == NULL) {
			_encrypt_deinit_aes(ctx->u.aes.ctx_enc);
			os_free(ctx);
			return NULL;
		}
		os_memcpy(ctx->u.aes.cbc, iv, AES_BLK_SZ);
		break;
						  ...
						  u8 * plain, size_t len)
{
	...
	case CRYPTO_CIPHER_ALG_AES:
		...
		blocks = len / AES_BLK_SZ;
		for (i = 0; i < blocks; i++) {
			os_memcpy(tmp, crypt, AES_BLK_SZ);
			aes_decrypt(ctx->u.aes.ctx_dec, crypt, plain);
			for (j = 0; j < AES_BLK_SZ; j++)
				plain[j] ^= ctx->u.aes.cbc[j];
			os_memcpy(ctx->u.aes.cbc, tmp, AES_BLK_SZ);
			plain += AES_BLK_SZ;
			crypt += AES_BLK_SZ;
		}
		break;

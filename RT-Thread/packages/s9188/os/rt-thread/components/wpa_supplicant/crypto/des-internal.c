...
void des_encrypt(const u8 * clear, const u8 * key, u8 * cypher)
{
	...
	work[0] = TILK_WPA_READ_BE32(clear);
	work[1] = TILK_WPA_READ_BE32(clear + 4);
	desfunc(work, ek);
	TILK_WPA_TO_BE32(cypher, work[0]);
	TILK_WPA_TO_BE32(cypher + 4, work[1]);
...
{
	u32 work[2];
	work[0] = TILK_WPA_READ_BE32(plain);
	work[1] = TILK_WPA_READ_BE32(plain + 4);
	desfunc(work, ek);
	TILK_WPA_TO_BE32(crypt, work[0]);
	TILK_WPA_TO_BE32(crypt + 4, work[1]);
}

void des_block_decrypt(const u8 * crypt, const u32 * dk, u8 * plain)
{
	u32 work[2];
	work[0] = TILK_WPA_READ_BE32(crypt);
	work[1] = TILK_WPA_READ_BE32(crypt + 4);
	desfunc(work, dk);
	TILK_WPA_TO_BE32(plain, work[0]);
	TILK_WPA_TO_BE32(plain + 4, work[1]);
}

...
//use the device as the key, and ssid info as the value, so we have key-value cache map
void wf_save_assoc_ssid(PADAPTER padapter, wf_u8 type)
{
	...
	while(try_cnt < 3) {
		...
		if(type == TRUE) {
			wf_memcpy(cache_info.dev_mac, nic_to_local_addr(padapter->nic_info), 6);
			wf_memcpy(&cache_info.ap_ssid, &mib_info->curr_ssid, sizeof(cache_info.ap_ssid));
			wf_memcpy(cache_info.ap_mac, ap_info->scan_info.bssid, 6);
			cache_info.ap_valid = type;
		} else {
...
int wf_get_assoc_ssid(PADAPTER padapter, wf_wlan_ssid_t *ssid)
{
	...
	while(try_cnt < 3) {
		...
		for(i=0; i<MP_MAX_DEVICE_NUM; i++) {
			...
			if(wf_memcmp(cache_info.dev_mac, nic_to_local_addr(padapter->nic_info), 6) == 0) {
				ssid->length = cache_info.ap_ssid.SSIDs[0].uSSIDLength;
				if(ssid->length <= WF_80211_MAX_SSID_LEN) {
					wf_memcpy(ssid->data, cache_info.ap_ssid.SSIDs[0].ucSSID, ssid->length);
				}
				find_flag = TRUE;
				break;
			}
...
static void wf_update_bss_list(void  *      adapter)
{
	...
	wf_wlan_mgmt_scan_que_for_rst_e scanned_ret = 0;
	...
	pnic_info = (nic_info_st *)padapter->nic_info;
...
static wf_wlan_mgmt_scan_que_node_t *wf_find_scan_info_by_ssid(PADAPTER adapter, PDOT11_SSID_LIST pDot11SSIDList)
{
    ...
    wf_wlan_mgmt_scan_que_for_begin(pnic_info, pscanned_info)
    {
        if(wf_memcmp(pscanned_info->ssid.data, pDot11SSIDList->SSIDs[0].ucSSID, pDot11SSIDList->SSIDs[0].uSSIDLength) == 0) {
...
{
	//wf_wlan_mgmt_scan_que_node_t *scan_info = pscan_info;
...
VOID wf_submit_disassoc_complete(PADAPTER pAdapter, ULONG Reason)
{
	...
	//clear AP info
	wf_memset(&ap_info->scan_info, 0, sizeof(ap_info->scan_info));
	ap_info->bcn_len = 0;
	ap_info->assoc_req_len = 0;
	ap_info->assoc_resp_len = 0;
	if(ap_info->valid)
		InterlockedDecrement(&ap_info->valid);
...
NDIS_STATUS wf_submit_link_qual(PADAPTER pAdapter)
{
    ...
    pEntry->ucLinkQuality = scan_info->signal_strength_scale;
...
NDIS_STATUS wf_set_start_scan(void *nic_info)
{
	...
	if (is_connected) {
        ...
        ret = wf_mlme_scan_start(pnic_info, SCAN_TYPE_PASSIVE,
                           pssid, ssid_num, NULL, 0, WF_MLME_FRAMEWORK_NDIS);
    } else {
...
NDIS_STATUS wf_set_scan_hidden_network(PADAPTER padapter, PDOT11_SSID_LIST pDot11SSIDList)
{
	...
	if(pDot11SSIDList == NULL) {
		LOG_E("param is NULL");
		return NDIS_STATUS_FAILURE;
	} else {
		ssid.length = pDot11SSIDList->SSIDs[0].uSSIDLength;
		if(ssid.length <= WF_80211_MAX_SSID_LEN) {
			wf_memcpy(ssid.data, pDot11SSIDList->SSIDs[0].ucSSID, ssid.length);
		} else {
...
NDIS_STATUS wf_set_start_assoc (PADAPTER adapter, PDOT11_SSID_LIST pDot11SSIDList)
{
	...
	if(adapter == NULL || pnic_info == NULL || mib_info == NULL) {
		LOG_E("param is NULL");
		return NDIS_STATUS_FAILURE;
	}
	...
	LOG_D("start, ssid=%s", pDot11SSIDList->SSIDs[0].ucSSID);

	scan_info = wf_find_scan_info_by_ssid(adapter, pDot11SSIDList);
	if(scan_info == NULL) {
		if(mib_info->hidden_network_enable) {
			if(wf_set_scan_hidden_network(adapter, pDot11SSIDList) != NDIS_STATUS_SUCCESS) {
			...
			LOG_E("hidden network is disable");
			return NDIS_STATUS_FAILURE; 
		}
	}
	
	//ap_info->scan_info = scan_info;
	wf_memcpy(&ap_info->scan_info, scan_info, sizeof(ap_info->scan_info));
	ap_info->bcn_len = 0;
	ap_info->assoc_req_len = 0;
	ap_info->assoc_resp_len = 0;
	if(!ap_info->valid)
		InterlockedIncrement(&ap_info->valid);
...
NDIS_STATUS wf_set_start_deassoc(void *adapter)
{
	...
	if(pAdapter == NULL || nic_info == NULL) {
		LOG_E("param is NULL");
		return NDIS_STATUS_FAILURE;
	}

	wf_save_assoc_ssid(adapter, FALSE);
...
VOID wf_set_auth(PADAPTER pAdapter)
{
	...
	wf_mib_info_t *mib_info = pAdapter->mib_info;
	sec_info_st *psec_info = NULL;
	DOT11_AUTH_ALGORITHM current_auth_type;
	...
	current_auth_type = mib_info->AuthAlgorithm;
...
NDIS_STATUS wf_get_bss_list(PADAPTER       pAdapter, PDOT11_BYTE_ARRAY pDot11ByteArray, ULONG TotalLength)
{
	...
	for(idx=0; idx<mib_info->bss_cnt; idx++) {
		...
		if (RemainingBytes >= BSSEntrySize) {
			...
			pDot11BSSEntry->lRSSI = pscanned_info->signal_strength_scale;//pscanned_info->signal_strength_scale - 100;
			pDot11BSSEntry->uLinkQuality = pscanned_info->signal_strength_scale;//pscanned_info->signal_qual;
			#if 0
			if(wf_80211_mgmt_ies_search(pscanned_info->ies + 12, ie_len_local, WF_80211_MGMT_EID_SSID, &ssid_info) == WF_RETURN_OK) {
				wf_memset(ssid_name, 0, 64);
				if(ssid_info->len < 64) {
					wf_memcpy(ssid_name, ssid_info->data, ssid_info->len);
					LOG_D("ssid=%10s, ss=%d, sq=%d", ssid_name, 
						pscanned_info->signal_strength, pscanned_info->signal_qual);
				} else {
					LOG_D("ssid_len is too long, len=%d", ssid_info->len);
				}
			}
			#endif
...
void wf_oids_adapt_init(void *adapter)
{
	...
	ap_info = wf_malloc(sizeof(wf_ap_info_t));
	if(ap_info == NULL){
		LOG_E("malloc ap info failed!!!\n");
		return;
	}

	memset(ap_info, 0, sizeof(wf_ap_info_t));

	KeInitializeSpinLock(&ap_info->lock);

	padapter->ap_info = ap_info;

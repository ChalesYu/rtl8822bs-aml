...
typedef struct _LOCAL_BSS_LIST
{
	...
	// Here the ies exclude the first fixed 12 bytes. And ie_len is also subtracted by 12.
	wf_u64 ie_len;
	wf_u8 ies[WF_WIN_80211_IES_SIZE_MAX];
...
typedef struct _ADAPTER
{
	...
	void					*mib_info; //To store MIB management infomation.
    ...
    ENUM_PARAM_OP_MODE_T               Dot11RunningMode;
	...
	/** Lower the IRQL and do the mailbox operation for Encryption/Decryption. */
	...
	// NDIS5 add.
	BOOLEAN		bBssidLocked;
	UCHAR       SpecifiedMacAddr[6];
	
    /* Packet pool and buffer pool */
    wf_u8                 *pucPayloadPool;     /* Starting address of the payload buffer pool */
    wf_u32                 u4PayloadPoolSz;    /* Total size of the payload buffer pool, in bytes */
    void                   *pvPktDescrHead;     /* Pointer to the head of packet descriptor list */
    wf_u32                 u4PktDescrFreeNum;  /* Currently available packet descriptors in the packet pool */
    wf_u32                 u4PktPoolSz;        /* Total acquired packets descriptors in the packet pool */

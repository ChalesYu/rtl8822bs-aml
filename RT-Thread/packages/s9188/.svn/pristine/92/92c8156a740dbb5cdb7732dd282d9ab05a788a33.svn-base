        ...
        if (plocal_info->work_mode == WF_INFRA_MODE) /* sta mode */
        {
            ...
            else /* group key */
            {
                ...
                if(wf_p2p_is_valid(pnic_info))
                {
                    ...
                    CFG80211_INFO("state:%s",wf_p2p_state_to_str(pwdinfo->p2p_state));
                               ...
                               struct key_params *))
{
    if(mac_addr)
    {
        CFG80211_DBG("mac addr: "WF_MAC_FMT,WF_MAC_ARG(mac_addr));
    }
    else
    {
        CFG80211_DBG();
...
void wf_cfg80211_indicate_connect (nic_info_st *pnic_info)
{
    ...
    if(wf_p2p_is_valid(pnic_info))
    {
        ...
        CFG80211_INFO("role=%s, p2p_state=%s, pre_p2p_state=%s\n",
                      wf_p2p_role_to_str(pwdinfo->role), wf_p2p_state_to_str(pwdinfo->p2p_state),
                      wf_p2p_state_to_str(pwdinfo->pre_p2p_state));
...
void wf_cfg80211_scan_complete(nic_info_st *pnic_info)
{
    ...
    wf_wlan_mgmt_scan_que_for_begin(pnic_info, pscan_que_node)
    {
        ...
        {
            wf_u8 i;
            for (i = 0; i < req->n_ssids - 1; i++)
            {
                if (req->ssids[i].ssid_len == pscan_que_node->ssid.length &&
                    !wf_memcmp(req->ssids[i].ssid, pscan_que_node->ssid.data,
                          ...
                          , struct cfg80211_scan_request *req)
{
    ...
    CFG80211_DBG("[%d] mac addr: "WF_MAC_FMT,pnic_info->ndev_id,WF_MAC_ARG(nic_to_local_addr(pnic_info)));
...
#define WL_ROCH_BACK_OP
    ...
    is_p2p_find = (duration < (pwdinfo->ext_listen_interval)) ? wf_true : wf_false;
...
#endif
    pwdinfo->ro_ch_duration = duration;
    pwdinfo->remain_ch = remain_ch;
...
#else
    cfg80211_remain_on_channel_expired(pwidev->pwidev, cfg_wfdirect_info->remain_on_ch_cookie, &cfg_wfdirect_info->remain_on_ch_channel, GFP_KERNEL);
#endif
    ...
    if (type >= 0)
    {
        switch (type)
        {
            ...
            case P2P_INVIT_RESP:
                ...
                //          wl_scan_deny_set(pwadptdata, 2000);

...
static int wpa_supplicant_select_config(struct wpa_supplicant *wpa_s)
{
	...
	wpa_printf(MSG_DEBUG, "Select network based on association " "information");
	...
	if (ssid->key_mgmt & (WPA_PSK_KEY_MGMT | WPA_IEEE8021X_KEY_MGMT |
						  WPA_WPA_NONE_KEY_MGMT |
...
static int wpa_supplicant_dynamic_keys(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->key_mgmt == WPA_NONE_KEY_MGMT ||
		wpa_s->key_mgmt == WPA_WPA_NONE_KEY_MGMT)
		return 0;

#ifdef IEEE8021X_EAPOL
	if (wpa_s->key_mgmt == WPA_IEEE8021X_NO_WPA_KEY_MGMT &&
		wpa_s->current_ssid &&
							  ...
							  struct wpa_ssid *ssid)
{
	...
	if (wpa_s->scard == NULL) {
		wpa_printf(MSG_WARNING, "Failed to initialize SIM " "(pcsc-lite)");
		return -1;
	}
...
#ifdef IEEE8021X_EAPOL
	if ((ssid->key_mgmt & WPA_IEEE8021X_NO_WPA_KEY_MGMT) &&
										 ...
										 struct wpa_scan_res *bss)
{
	...
	rsn_ie = wpa_scan_get_ie(bss, WLAN_RSN_EID);
	while ((ssid->proto & WPA_RSN_PROTO) && rsn_ie) {
		proto_match++;
		...
		if (!(ie.proto & ssid->proto)) {
			wpa_printf(MSG_DEBUG, "   skip RSN IE - proto " "mismatch");
			break;
		}

		if (!(ie.pairwise_cipher & ssid->pairwise_cipher)) {
			wpa_printf(MSG_DEBUG, "   skip RSN IE - PTK cipher " "mismatch");
			break;
		}

		if (!(ie.group_cipher & ssid->group_cipher)) {
			wpa_printf(MSG_DEBUG, "   skip RSN IE - GTK cipher " "mismatch");
			break;
		}

		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			wpa_printf(MSG_DEBUG, "   skip RSN IE - key mgmt " "mismatch");
			break;
		}
	...
	while ((ssid->proto & WPA_WPA_PROTO) && wpa_ie) {
		proto_match++;
		...
		if (!(ie.proto & ssid->proto)) {
			wpa_printf(MSG_DEBUG, "   skip WPA IE - proto " "mismatch");
			break;
		}

		if (!(ie.pairwise_cipher & ssid->pairwise_cipher)) {
			wpa_printf(MSG_DEBUG, "   skip WPA IE - PTK cipher " "mismatch");
			break;
		}

		if (!(ie.group_cipher & ssid->group_cipher)) {
			wpa_printf(MSG_DEBUG, "   skip WPA IE - GTK cipher " "mismatch");
			break;
		}

		if (!(ie.key_mgmt & ssid->key_mgmt)) {
			wpa_printf(MSG_DEBUG, "   skip WPA IE - key mgmt " "mismatch");
			break;
		}
...
static struct wpa_bss *wpa_supplicant_select_bss_wpa(struct wpa_supplicant
													 *wpa_s,
													 ...
													 **selected_ssid)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		ie = wpa_scan_get_ie(bss, WLAN_SSID_EID);
		ssid_ = ie ? ie + 2 : (u8 *) "";
		ssid_len = ie ? ie[1] : 0;
		...
		ie = wpa_scan_get_ie(bss, WLAN_RSN_EID);
		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			...
			if (check_ssid &&
				(ssid_len != ssid->ssid_len ||
				 os_memcompare(ssid_, ssid->ssid, ssid_len) != 0)) {
				wpa_printf(MSG_DEBUG, "   skip - " "SSID mismatch");
				continue;
			}

			if (ssid->bssid_set &&
				os_memcompare(bss->bssid, ssid->bssid, ETH_ALEN) != 0) {
			...
			if (!freq_allowed(ssid->freq_list, bss->freq)) {
				wpa_printf(MSG_DEBUG, "   skip - " "frequency not allowed");
				continue;
			}

			wpa_printf(MSG_DEBUG, "   selected WPA AP "
					   MACSTR " ssid='%s'",
					   MAC2STR(bss->bssid), wpa_ssid_txt(ssid_, ssid_len));
			*selected_ssid = ssid;
...
static struct wpa_bss *wpa_supplicant_select_bss_non_wpa(struct wpa_supplicant
														 *wpa_s,
														 struct wpa_scan_results
														 *scan_res,
														 struct wpa_ssid *group,
														 ...
														 **selected_ssid)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		for (ssid = group; ssid; ssid = ssid->pnext) {
			...
			if (!(ssid->key_mgmt & WPA_NONE_KEY_MGMT) &&
				!(ssid->key_mgmt & WPA_KEY_MGMT_WPS) &&
				!(ssid->key_mgmt & WPA_IEEE8021X_NO_WPA_KEY_MGMT)) {
			...
			if ((ssid->key_mgmt &
				 (WPA_IEEE8021X_KEY_MGMT | WPA_PSK_KEY_MGMT |
				  WPA_KEY_MGMT_FT_IEEE8021X | WPA_KEY_MGMT_FT_PSK |
				  WPA_KEY_MGMT_IEEE8021X_SHA256 |
				  WPA_KEY_MGMT_PSK_SHA256)) &&
				(wpa_ie_len != 0 || rsn_ie_len != 0)) {
				wpa_printf(MSG_DEBUG, "   skip - " "WPA network");
				continue;
			}
			...
			if (bss->caps & IEEE80211_CAP_IBSS) {
				wpa_printf(MSG_DEBUG, "   skip - " "IBSS (adhoc) network");
				continue;
			}
												   ...
												   **selected_ssid)
{
	...
	while (selected == NULL) {
		for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
			selected =
				wpa_supplicant_select_bss(wpa_s, scan_res,
							...
							struct wpa_bss *selected, struct wpa_ssid *ssid)
{
	...
	if (wpa_s->reassociate ||
		(os_memcompare(selected->bssid, wpa_s->bssid, ETH_ALEN) != 0 &&
		 (wpa_s->wpa_state != WPA_ASSOCIATING ||
		  os_memcompare(selected->bssid, wpa_s->pending_bssid, ETH_ALEN) !=
		  0))) {
...
static struct wpa_ssid *wpa_supplicant_pick_new_network(struct wpa_supplicant
														*wpa_s)
{
	int prio;
	struct wpa_ssid *ssid;
...
static void wpa_supplicant_rsn_preauth_scan_results(struct wpa_supplicant
													*wpa_s,
													struct wpa_scan_results
													*scan_res)
{
	...
	for (i = scan_res->num - 1; i >= 0; i--) {
		...
		ssid = wpa_scan_get_ie(r, WLAN_SSID_EID);
		if (ssid == NULL)
			continue;

		rsn = wpa_scan_get_ie(r, WLAN_RSN_EID);
		if (rsn == NULL)
			continue;
									   ...
									   struct wpa_scan_results *scan_res)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		const u8 *ie;
		if (os_memcompare(res->bssid, wpa_s->bssid, ETH_ALEN) != 0)
			continue;

		ie = wpa_scan_get_ie(res, WLAN_SSID_EID);
		if (ie == NULL)
			continue;
		if (ie[1] != wpa_s->current_ssid->ssid_len ||
			os_memcompare(ie + 2, wpa_s->current_ssid->ssid, ie[1]) != 0)
			continue;
		current_bss = res;
		break;
	}
	...
	if (wpa_s->current_ssid->bssid_set &&
		os_memcompare(selected->bssid, wpa_s->current_ssid->bssid, ETH_ALEN) ==
		0) {
										  ...
										  union wpa_event_data *data)
{
	...
	while (p && l >= 2) {
		...
		if ((p[0] == WLAN_VENDOR_SPECIFIC_EID && p[1] >= 6 &&
			 (os_memcompare(&p[2], "\x00\x50\xF2\x01\x01\x00", 6) == 0)) ||
			(p[0] == WLAN_RSN_EID && p[1] >= 2)) {
	...
	while (p && l >= 2) {
		...
		if (p[0] == WLAN_MOBILITY_FIELD_EID && p[1] >= MOBILITY_DOMAIN_ID_LEN) {
			wpa_s->sme.ft_used = 1;
	...
	while (p && l >= 2) {
		...
		if (!wpa_found &&
			p[0] == WLAN_VENDOR_SPECIFIC_EID && p[1] >= 6 &&
			os_memcompare(&p[2], "\x00\x50\xF2\x01\x01\x00", 6) == 0) {
			wpa_found = 1;
			wpa_sm_set_ap_wpa_ie(wpa_s->wpa, p, len);
		}

		if (!rsn_found && p[0] == WLAN_RSN_EID && p[1] >= 2) {
			rsn_found = 1;
									   ...
									   union wpa_event_data *data)
{
	...
	if ((wpa_s->drv_flags & WPA_DRIVER_FLAGS_USER_SPACE_MLME) ||
		(wpa_drv_get_bssid(wpa_s, bssid) >= 0 &&
		 os_memcompare(bssid, wpa_s->bssid, ETH_ALEN) != 0)) {
		wpa_msg(wpa_s, MSG_DEBUG, "Associated to a new BSS: BSSID="
				MACSTR, MAC2STR(bssid));
		bssid_changed = os_memcompare(wpa_s->bssid, bssid, ETH_ALEN);
		...
		if (wpa_supplicant_select_config(wpa_s) < 0) {
			wpa_supplicant_disassociate(wpa_s, WLAN_DEAUTH_LEAVING_CAUSE);
			return;
		}
	...
	if (wpa_s->pending_eapol_rx) {
		...
		if (age.sec == 0 && age.usec < 100000 &&
			os_memcompare(wpa_s->pending_eapol_rx_src, bssid, ETH_ALEN) == 0) {
	...
	if (wpa_s->current_ssid != wpa_s->bgscan_ssid) {
		...
		if (wpa_s->current_ssid && wpa_s->current_ssid->bgscan) {
			if (bgscan_init(wpa_s, wpa_s->current_ssid)) {
				wpa_printf(MSG_DEBUG, "Failed to initialize " "bgscan");
										  ...
										  u16 reason_code)
{
	...
	if (wpa_s->key_mgmt == WPA_WPA_NONE_KEY_MGMT) {
	...
	wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_DISCONNECTED "bssid=" MACSTR
			" reason=%d", MAC2STR(bssid), reason_code);
	...
	if ((wpa_s->last_michael_mic_error &&
		...
		if (wpa_s->mic_errors_seen) {
			...
			if (os_get_random(rval, sizeof(rval)) < 0)
				...
				sec = TILK_WPA_READ_BE32(rval) % 60;
						 ...
						 size_t len)
{
	...
	if (os_memcompare(sta_addr, wpa_s->own_addr, ETH_ALEN) != 0) {
						  ...
						  union wpa_event_data *data)
{
	...
	case EVENT_RX_ACTION:
		...
		if (data->rx_action.category == WLAN_ACTION_FT) {
			ft_rx_action(wpa_s, data->rx_action.data, data->rx_action.len);
			break;
		}
#endif
		break;
	...
	case EVENT_EAPOL_RX:
		wpa_supplicant_rx_eapol(wpa_s, data->eapol_rx.src,
								data->eapol_rx.data, data->eapol_rx.data_len);
		break;

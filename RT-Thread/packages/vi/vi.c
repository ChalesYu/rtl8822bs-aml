...
static void new_screen(int, int);  // malloc virtual screen memory
#if !ENABLE_FEATURE_VI_UNDO
...
static int find_range(char **start, char **stop, int cmd);  // return pointers for an object
...
#endif
#if ENABLE_FEATURE_VI_YANKMARK
static char *text_yank(char *, char *, int, int);    // save copy of "p" into a register
...
static void undo_push(char *, unsigned, int); // push an operation on the undo stack
...
static void colon(char *buf)
{
    ...
    *cmdend = '\0';
    ...
    if (i == 0) {       // :123CR goto line #123
        ...
        n = cmdline_filecnt - options.optind - 1;
        if (*cmd == 'q' && n > 0) {

...
#ifndef RT_DFS_LFS_DRIVERS
#define RT_DFS_LFS_DRIVERS    1
#endif

#if (RT_DFS_LFS_DRIVERS < 1)
#error "#define RT_DFS_LFS_DRIVERS must > 0"
#endif
...
static struct _dfs_lfs_s* _lfs_mount_tbl[RT_DFS_LFS_DRIVERS] = {0};
static struct rt_mutex __lfs_lock;

#define lfs_dfs_lock()          rt_mutex_take(&__lfs_lock, RT_WAITING_FOREVER);
#define lfs_dfs_unlock()        rt_mutex_release(&__lfs_lock);
...
/* results:
*  -1, no space to install littlefs driver
...
static int _get_disk(rt_device_t dev_id)
{
  ...
  if (dev_id == RT_NULL)
  {
    for (index = 0; index < RT_DFS_LFS_DRIVERS; index ++)
    {
...
int dfs_lfs_init(void)
{
  ...
  rt_mutex_init(&__lfs_lock, "lfsmtx", RT_IPC_FLAG_FIFO);

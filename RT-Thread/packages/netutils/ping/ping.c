...
#define PING_RCV_TIMEO (3*RT_TICK_PER_SECOND)
...
rt_err_t ping(char* target_name, rt_uint32_t times, rt_size_t size)
{
    ...
    rt_uint32_t send_times, failed_times;
    ...
    while (1)
    {
        if (ping_send(s, &target_addr, size) == ERR_OK)
        {
            ...
            if ((recv_len = ping_recv(s, &ttl)) >= 0)
            {
                rt_kprintf("%d bytes from %s icmp_seq=%d ttl=%d time=%d ms\n", recv_len, inet_ntoa(ina), send_times,
                        ttl, (rt_tick_get() - recv_start_tick) * 1000 / RT_TICK_PER_SECOND);
            }
            else
            {
        ...
        if (send_times >= times)
        {
            ...
            rt_kprintf("Ping %d times, %d times failed\n", send_times, failed_times);
            break;
        }
...
int cmd_ping(int argc, char **argv)
{
    if (argc == 1)
    {
        rt_kprintf("Please input: ping <host address> <times>\r\n");
    } 
    else if(argc == 3)
    {
      ...
      if(times > 0) {
        ...
        rt_kprintf("Ping times input error\r\n");
      }
    }
    else if(argc == 4)
    {

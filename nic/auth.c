...
wf_pt_rst_t wf_auth_sta_thrd (wf_pt_t *pt, nic_info_st *pnic_info, int *prsn)
{
    ...
    PT_WAIT_WHILE(pt, wf_os_api_sema_try(&pauth_info->sema));
    do
    {
...
wf_inline static int auth_msg_deinit (wf_msg_que_t *pmsg_que)
{
...
int wf_auth_init (nic_info_st *pnic_info)
{
    ...
    wf_os_api_sema_init(&pauth_info->sema, 1);
    pauth_info->brun = wf_false;

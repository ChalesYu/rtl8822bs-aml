...
int scan_setting (nic_info_st *pnic_info)
{
    ...
    else
    {
        ...
        if (wf_80211_is_valid_bssid(pscan_info->preq->bssid))
        {
            ...
            {
                if (!wf_memcmp(pscanned_info->bssid, pscan_info->preq->bssid,
                               sizeof(pscanned_info->bssid)))
                {
                    SCAN_DBG("found bss in scan queue");
                    break;
                }
            }
            wf_wlan_scanned_each_end(pnic_info, &rst);
            ...
            if (rst == WF_WLAN_SCANNED_EACH_RET_BREAK)
            {
                ...
                if (pscan_info->preq->ch_num)
                {
                    ...
                    {
                        if (pscanned_info->channel == pscan_info->preq->ch_map[i])
                        {
                            bfix_ch = wf_true;
                            break;
                        }
                ...
                if (bfix_ch)
                {
                    ...
                    SCAN_DBG("fix scan channel number: %d",
                             pscanned_info->channel);
                    pscan_info->preq->ch_num = 1;
                    pscan_info->preq->ch_map[0] = pscanned_info->channel;
                    /* set scan filte */
...
wf_inline static wf_bool is_tx_buf_empty (nic_info_st *pnic_info)
{
    return wf_io_write_data_queue_check(pnic_info) == wf_true &&
           wf_mcu_check_tx_buff(pnic_info) == WF_RETURN_OK;
}
...
wf_pt_rst_t wf_scan_thrd (wf_pt_t *pt, nic_info_st *pnic_info, int *prsn)
{
    wf_scan_info_t *pscan_info;
    ...
    PT_WAIT_WHILE(pt, wf_os_api_sema_try(&pscan_info->sema));
    ...
    do
    {
        ...
        if (rst)
        {
            ...
            wf_os_api_sema_post(&pscan_info->sema);
            *prsn = -1;
            PT_EXIT(pt);
        }
    ...
    /* clearup tx buffer */
    SCAN_DBG("wait until tx buffer empty ......");
    wf_timer_set(&pscan_info->timer, 1000);
    while (!is_tx_buf_empty(pnic_info))
    {
...
wf_inline static int scan_msg_deinit (wf_msg_que_t *pmsg_que)
{
...
int wf_scan_init (nic_info_st *pnic_info)
{
    ...
    wf_os_api_sema_init(&pscan_info->sema, 1);

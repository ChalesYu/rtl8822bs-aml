...
int wf_fw_download(nic_info_st *nic_info)
{
    ...
    if (nic_info->nic_type == NIC_USB)
    {
        ...
        for (i = 0; i < block; i++)
        {
            ret = nic_info->nic_write(nic_info->hif_node, 0, WF_START_ADDR, (wf_u8 *)&fwdl_info->fw0[i * 64], 64);
            if (ret < 0)
            {
        ...
        if (remain)
        {
            nic_info->nic_write(nic_info->hif_node, 0, WF_START_ADDR, (wf_u8 *)&fwdl_info->fw0[i * 64], remain);
            if (ret < 0)
            {
                LOG_E("===>nic_write error");
                return -1;
    ...
    else
    {
        ...
        for (i = 0; i < block; i++)
        {
            ret = nic_info->nic_write(nic_info->hif_node, 0, WF_START_ADDR, (wf_u8 *)&fwdl_info->fw0[i * 4], 4);
            if (ret < 0)
            {
                LOG_E("===>nic_write error");
                return -1;
        ...
        if (remain)
        {
            nic_info->nic_write(nic_info->hif_node, 0, WF_START_ADDR, (wf_u8 *)&fwdl_info->fw0[i * 4], remain);
            if (ret < 0)
            {
                LOG_E("===>nic_write error");
                return -1;
    ...
    if (nic_info->nic_type == NIC_USB)
    {
        ...
        if(0 != wf_io_write_firmware(nic_info, 0, fwdl_info->fw0, fwdl_info->fw0_size)) {
            return -1;
        }
        if(0 != wf_io_write_firmware(nic_info, 1, fwdl_info->fw1, fwdl_info->fw1_size)) {
            return -1;
        }
    }
    else  // sdio
    {
        ...
        if(nic_info->fwdl_info.fw_rom_type) 
        {
            value8 = wf_io_read8(nic_info, 0xf4,NULL);
            value8 &= 0xFE;
            ret = wf_io_write8(nic_info, 0xf4, value8);

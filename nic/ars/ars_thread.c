...
//    wf_u8   vht_en = ((pars->RxRate) >= ODM_RATEVHTSS1MCS0) ? 1 : 0;
    ...
    {
        ARS_THD_PRT("[CCK AGC Report] LNA_idx = %d, VGA_idx = %d",pars->cck_lna_idx, pars->cck_vga_idx);        
    } 
    else 
    {
        ARS_THD_PRT("[OFDM AGC Report] { 0x%x, 0x%x, 0x%x, 0x%x }",
            pars->ofdm_agc_idx[0], pars->ofdm_agc_idx[1], pars->ofdm_agc_idx[2], pars->ofdm_agc_idx[3]);    
    }

    ARS_THD_PRT("RSSI: { %d,  %d,  %d,  %d },    RxRate: { %s%s%s%s%d%s}",
...
wf_s32 phydm_NoisyDetection(ars_st *pars)
{
    ...
    {
        case 0:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=0%%\n");
            break;
        case 1:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=6.25%%\n");
            break;
        case 2:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=12.5%%\n");
            break;
        case 3:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=18.75%%\n");
            break;
        case 4:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=25%%\n");
            break;
        case 5:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=31.25%%\n");
            break;
        case 6:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=37.5%%\n");
            break;
        case 7:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=43.75%%\n");
            break;
        case 8:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=50%%\n");
            break;
        case 9:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=56.25%%\n");
            break;
        case 10:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=62.5%%\n");
            break;
        case 11:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=68.75%%\n");
            break;
        case 12:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=75%%\n");
            break;
        case 13:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=81.25%%\n");
            break;
        case 14:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=87.5%%\n");
            break;
        case 15:
            ARS_THD_PRT ("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=93.75%%\n");            
            break;
        case 16:
            ARS_THD_PRT("[NoisyDetection] Total_FA_Cnt/Total_CCA_Cnt=100%%\n");
            break;
        default:
            ARS_THD_PRT("[NoisyDetection] Unknown Value!! Need Check!!\n");            
    }

    ARS_THD_PRT("[NoisyDetection] Total_CCA_Cnt=%d, Total_FA_Cnt=%d, NoisyDecision_Smooth=%d, Score=%d, Score_Smooth=%d, pars->NoisyDecision=%d\n",

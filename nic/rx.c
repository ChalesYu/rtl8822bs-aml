...
#define C2H_ID(_c2h)        LE_BITS_TO_1BYTE(((wf_u8*)(_c2h)), 0, 8)
#define C2H_SEQ(_c2h)       LE_BITS_TO_1BYTE(((wf_u8*)(_c2h)) + 1, 0, 8)
#define C2H_PAYLOAD(_c2h)   (((wf_u8*)(_c2h)) + 2)


inline wf_u8 *pkt_push(prx_pkt_t prx_pkt, int size)
{
...
inline static wf_u8 *pkt_put(prx_pkt_t prx_pkt, int size)
{
    wf_u8 *ptmp;
...
inline static void pkt_reserve(prx_pkt_t prx_pkt, int size)
{
    prx_pkt->pdata += size;
    prx_pkt->ptail += size;
}

inline void free_rx_pkt(prx_pkt_t prx_pkt, wf_bool bfreeSkb)
{
...
inline int wf_rx_data_len_check(nic_info_st *pnic_info,wf_u8 *pbuf, wf_u16 skb_len)
{
    int ret;
    wf_u16 pkt_len;
    rx_info_t *rx_info = NULL;
...
inline wf_u16 wf_rx_get_pkt_len_and_check_valid(wf_u8 *buf, wf_u16 remain, wf_bool *valid, wf_bool *notice)
{
    wf_u16 pkt_len;
...
int rx_reorder_upload(recv_ba_ctrl_st   *ba_order)
{
    ...
    while(1)
    {
        ...
        if (!SN_LESS(ba_order->indicate_seq, get_reorder->seq_num))
        {
            ...
            if(de_reorder->pskb)
            {
                ba_order->upload_func(nic_info->ndev, de_reorder->pskb);
            }

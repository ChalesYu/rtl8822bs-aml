                         ...
                         wf_80211_frame_e type, wf_80211_statuscode_e status)
{
    ...
    if (pwdn_info->wmm_enable)
        wf_memcpy(&pvar[var_len], &pcur_network->pwmm,
                  sizeof(wf_80211_wmm_param_ie_t));
         ...
         ofs += WF_OFFSETOF(wf_80211_mgmt_ie_t, data) + pie->len)
    {
        ...
        else if (pie->element_id == WF_80211_MGMT_EID_VENDOR_SPECIFIC)
        {
            ...
            if (!ret)
            {
                ...
                wf_memcpy(&pwdn_info->wmm_info, pie->data, pie->len);
            }
        }
...
static int associate_xmit_frame (nic_info_st *nic_info)
{
    ...
    {
        if (sec_info->ndisauthtype == wf_ndis802_11AuthModeWPA ||
    ...
    {
        if (sec_info->ndisauthtype == wf_ndis802_11AuthModeWPA2 ||
...
wf_pt_rst_t wf_assoc_sta_thrd (wf_pt_t *pt, nic_info_st *pnic_info, int *prsn)
{
    ...
    PT_WAIT_WHILE(pt, wf_os_api_sema_try(&passoc_info->sema));
    ...
    do
    {
        if (wf_msg_pop(pmsg_que, &pmsg))
        {
            ...
            wf_os_api_sema_post(&passoc_info->sema);
            *prsn = -1;
            PT_EXIT(pt);
        }
...
wf_inline static int assoc_msg_deinit (wf_msg_que_t *pmsg_que)
{
...
int wf_assoc_init (nic_info_st *pnic_info)
{
    ...
    wf_os_api_sema_init(&passoc_info->sema, 1);

...
wf_bool wf_xmit_frame_init(nic_info_st *nic_info, struct xmit_frame *pxmitframe, wf_u8 *msdu_buf, int msdu_len)
{
    ...
    pxmitframe->ether_type = BE_P2BYTE_TO_HOST_2BYTE(&pethhdr->type);
...
static void txdesc_vcs_fill(nic_info_st *nic_info, struct xmit_frame *pxmitframe, wf_u8 * ptxdesc)
{
    ...
    {
        case RTS_CTS:
            SET_BITS_TO_LE_4BYTE(ptxdesc + 8, 27, 2, 1);
            break;
        case CTS_TO_SELF:
            SET_BITS_TO_LE_4BYTE(ptxdesc + 8, 27, 2, 2);
            break;
        case NONE_VCS:
        default:
            break;
    ...
    if (pxmitframe->vcs_mode)
    {
        ...
        if (pwdn->short_preamble == wf_true)
            SET_BITS_TO_LE_4BYTE(ptxdesc + 8, 29, 1, 1);

        if (pxmitframe->ht_en)
        {
            SET_BITS_TO_LE_4BYTE(ptxdesc + 8, 30, 2, txdesc_scmapping_get(nic_info, pxmitframe));
        }
    }
    else
    {
                            ...
                            wf_u8 * ptxdesc)
{
    ...
    if (pxmitframe->ht_en)
    {
        ...
        /* set DSC */
        SET_BITS_TO_LE_4BYTE(ptxdesc + 16, 13, 2, txdesc_scmapping_get(nic_info, pxmitframe));
#else
...
static void txdesc_fill(struct xmit_frame *pxmitframe, wf_u8 * pbuf, wf_bool bSendAck)
{
    ...
    SET_BITS_TO_LE_4BYTE(pbuf + 16, 0, 5, pwdn->wdn_id);
    if (pwdn->raid <= 8)
    {
        ...
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 6, 3, pwdn->raid);
    }
    else
    {
        /* set rate mode, mgmt frame use adp mode */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 5, 1, 1);
        /* set RATE ID, the number is raid - 9 */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 9, 3, pwdn->raid - 9);
    }
    /* set QOS QUEUE  */
    SET_BITS_TO_LE_4BYTE(pbuf + 12, 6, 5, pxmitframe->qsel);
    /* set SEQ */
    SET_BITS_TO_LE_4BYTE(pbuf, 19, 12, pxmitframe->seqnum);
    /* set secture type */
    SET_BITS_TO_LE_4BYTE(pbuf + 12, 3, 3, sectype_to_hwdesc_get(pxmitframe));
            ...
            (hw_info->use_fixRate != wf_true))
    {
        ...
        {
            /* set AGG Enable */
            SET_BITS_TO_LE_4BYTE(pbuf + 12, 0, 1, 1);
        }
        else
        {
            /* set AGG Break */
            SET_BITS_TO_LE_4BYTE(pbuf + 12, 19, 1, 1);
        }
    ...
    else
    {
        ...
        if (pwdn->short_preamble== wf_true)
                SET_BITS_TO_LE_4BYTE(pbuf + 8, 17, 1, 1);

        /* set USE_RATE */
        SET_BITS_TO_LE_4BYTE(pbuf + 8, 16, 1, 1);

        /* set TX RATE */
        SET_BITS_TO_LE_4BYTE(pbuf + 8, 18, 7, wf_mrate_to_hwrate(pwdn->tx_rate));
    }
#ifdef CONFIG_SOFT_TX_AGGREGATION
    SET_BITS_TO_LE_4BYTE(pbuf + 12, 24, 8, pxmitframe->agg_num);
#endif

    if (bSendAck == wf_true)
    {
        /* set SPE_RPT */
        SET_BITS_TO_LE_4BYTE(pbuf + 12, 21, 1, 1);
        /* set SW_DEFINE */
        SET_BITS_TO_LE_4BYTE(pbuf + 4, 16, 12, pwdn->wdn_xmitpriv.txseq_tid[pxmitframe->qsel]);
    }
...
inline wf_bool wf_need_stop_queue(nic_info_st *nic_info)
{
    tx_info_st *tx_info = nic_info->tx_info;
...
inline wf_bool wf_need_wake_queue(nic_info_st *nic_info)
{
    tx_info_st *tx_info = nic_info->tx_info;

    if (tx_info->xmitFrameCtl == 1)
    {
        if (tx_info->free_xmitframe_cnt > 255)
        {
...
struct xmit_buf *wf_xmit_buf_new(tx_info_st *tx_info)
{
    ...
    if (wf_que_is_empty(pfree_xmitbuf_queue) == wf_true)
    {
        pxmitbuf = NULL;
    }
    else
    {
        phead = wf_que_head(pfree_xmitbuf_queue);
...
wf_bool wf_xmit_buf_delete(tx_info_st *tx_info, struct xmit_buf *pxmitbuf)
{
    ...
    wf_list_insert_tail(&(pxmitbuf->list),
                        wf_que_head(pfree_xmitbuf_queue));
    tx_info->free_xmitbuf_cnt++;
    wf_lock_unlock(&pfree_xmitbuf_queue->lock);
...
struct xmit_frame *wf_xmit_frame_new(tx_info_st *tx_info)
{
    ...
    if (wf_que_is_empty(pfree_xmit_queue) == wf_true)
    {
#if 0
    ...
    {
        phead = wf_que_head(pfree_xmit_queue);
...
wf_bool wf_xmit_frame_delete(tx_info_st *tx_info, struct xmit_frame * pxmitframe)
{
    ...
    wf_list_insert_tail(&pxmitframe->list, wf_que_head(free_queue));
    tx_info->free_xmitframe_cnt++;
...
wf_bool wf_xmit_frame_enqueue(tx_info_st *tx_info, struct xmit_frame * pxmitframe)
{
    ...
    wf_list_insert_tail(&pxmitframe->list, wf_que_head(queue));
    tx_info->free_xmitframe_cnt++;
...
int wf_nic_beacon_xmit(nic_info_st *nic_info, struct xmit_buf *pxmitbuf, wf_u16 len)
{
    ...
    wf_io_write_data(nic_info,1, pbuf, len, wf_quary_addr(QSLT_BEACON), (void *)wf_xmit_extbuf_delete, tx_info, pxmitbuf);
...
int wf_nic_mgmt_frame_xmit_with_ack(nic_info_st *nic_info, wdn_net_info_st *wdn, struct xmit_buf *pxmitbuf, wf_u16 len)
{
    ...
    if (wdn)
    {
        ...
        wdn->wdn_xmitpriv.txseq_tid[QSLT_MGNT] &= 0xFFF;
        /* set SEQ */
        SET_BITS_TO_LE_4BYTE(pbuf, 19, 12, wdn->wdn_xmitpriv.txseq_tid[QSLT_MGNT]);

        /* set mac id or sta index */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 0, 5, wdn->wdn_id);
        /* set DBW */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 12, 1, wdn->bw_mode);
        ...
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 5, 1, 0);
        /* set RATE ID */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 6, 3,  wdn->raid);
        /* set TX RATE */
        SET_BITS_TO_LE_4BYTE(pbuf + 8, 18, 7, wf_mrate_to_hwrate(wdn->tx_rate));

        /* set SPE_RPT */
        SET_BITS_TO_LE_4BYTE(pbuf + 12, 21, 1, 1);
        /* set SW_DEFINE */
        SET_BITS_TO_LE_4BYTE(pbuf + 4, 16, 12, wdn->wdn_xmitpriv.txseq_tid[QSLT_MGNT]);
    ...
    {
        /* set SEQ */
        SET_BITS_TO_LE_4BYTE(pbuf, 19, 12, 0);
        /* set mac id or sta index */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 0, 5, 0);
        /* set DBW */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 12, 1, CHANNEL_WIDTH_20);
        /* set DSC */
        SET_BITS_TO_LE_4BYTE(pbuf + 16, 13, 2, HT_DATA_SC_DONOT_CARE);
    ...
    ret = wf_io_write_data(nic_info, 1, pbuf, len, wf_quary_addr(QSLT_MGNT), (void *)wf_xmit_extbuf_delete, tx_info, pxmitbuf);
    if (ret == 0)
    {
        timeout = 0;
